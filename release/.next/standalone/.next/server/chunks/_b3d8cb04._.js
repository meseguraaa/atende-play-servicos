module.exports = [
    ...
    (e) => {
        ...
        var h = e.i(377404),
            ...
            S = e.i(29173);
        function T(e, t = 200) {
            return g.NextResponse.json({ ok: !0, data: e }, { status: t });
        }
        function b(e, t = 400) {
        ...
        function _(e) {
            ...
            return !Number.isInteger(n) ||
                ...
                s > 31
                ? null
                : new Date(Date.UTC(n, r - 1, s, 12, 0, 0, 0));
        }
        async function x() {
            let e = await (0, E.requireProfessionalSession)(),
            ...
            if (
                !a ||
                !(await A.prisma.professionalUnit.findFirst({
                    where: {
                        companyId: t,
                        professionalId: i,
                        unitId: a,
        ...
        async function C() {
            try {
                let e = await x(),
                    t = await A.prisma.professionalDailyAvailability.findMany({
                        where: {
                ...
                return T({
                    exceptions: t.map((e) => ({
                        id: e.id,
                        dateISO: (0, w.format)(e.date, 'yyyy-MM-dd'),
                        type:
                            e.type ===
                            S.ProfessionalDailyAvailabilityType.DAY_OFF
                                ? 'DAY_OFF'
                                : 'CUSTOM',
        ...
        async function P(e) {
            try {
                let t = await x(),
                    ...
                    a = String(i?.dateISO ?? ''),
                    n = i?.mode;
                if (!a) return b('dateISO é obrigatório.', 400);
                if ('FULL_DAY' !== n && 'PARTIAL' !== n)
                    return b('mode inválido. Use FULL_DAY ou PARTIAL.', 400);
                let r = _(a);
                if (!r) return b('dateISO inválido. Use yyyy-MM-dd.', 400);
                let s = Array.isArray(i?.intervals) ? i.intervals : [];
                if ('PARTIAL' === n) {
                    if (!s.length)
                        return b('Adicione pelo menos 1 intervalo.', 400);
                    for (let e of s)
                        if (
                            !e?.startTime ||
                            !e?.endTime ||
                ...
                let o =
                        'FULL_DAY' === n
                            ? S.ProfessionalDailyAvailabilityType.DAY_OFF
                            : S.ProfessionalDailyAvailabilityType.CUSTOM,
                    l = await A.prisma.$transaction(async (e) => {
                        let i = await e.professionalDailyAvailability.upsert({
                            where: {
                        ...
                        return (
                            await e.professionalDailyTimeInterval.deleteMany({
                                where: { dailyAvailabilityId: i.id },
                            }),
                            'PARTIAL' === n &&
                                (await e.professionalDailyTimeInterval.createMany(
                                    {
                                        data: s.map((e) => ({
                                            dailyAvailabilityId: i.id,
                                            startTime: e.startTime,
                                            endTime: e.endTime,
                                        })),
                                    }
                ...
                return (
                    (0, R.revalidatePath)('/professional/availability'),
                    T({ id: l.id })
                );
            } catch (e) {
                return b(
        ...
        async function O(e) {
            try {
                let t = await x(),
                    { searchParams: i } = new URL(e.url),
                    a = i.get('dateISO');
                if (!a) return b('dateISO é obrigatório.', 400);
                let n = _(a);
                if (!n) return b('dateISO inválido. Use yyyy-MM-dd.', 400);
                return (
                    await A.prisma.professionalDailyAvailability.delete({
                        where: {
                            professionalId_unitId_date: {
                                professionalId: t.professionalId,
                                unitId: t.unitId,
                ...
                if (
                    'P2025' === e ||
                    t.includes('Record to delete does not exist')
                )
                    return T({ deleted: !0 });
                return b(
                    i?.message === 'missing_company'
                        ? 'Sessão sem companyId.'
                        : i?.message === 'missing_professional'
                          ? 'Sessão sem professionalId.'
                          : i?.message === 'missing_active_unit'
                            ? 'Este profissional não possui unidade ativa vinculada.'
        ...
        e.s(['DELETE', () => O, 'GET', () => C, 'POST', () => P], 931858);
        var D = e.i(931858);
        let N = new t.AppRouteRouteModule({
                definition: {
                    kind: i.RouteKind.APP_ROUTE,
                    page: '/api/professional/availability/exceptions/route',
                    pathname: '/api/professional/availability/exceptions',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/professional/availability/exceptions/route.ts',
        ...
        async function L(e, t, a) {
            ...
            let g = '/api/professional/availability/exceptions/route';
            g = g.replace(/\/index$/, '') || '/';
            let R = await N.prepare(e, t, {
                srcPage: g,
            ...
            let {
                ...
                M = !!(b.dynamicRoutes[U] || b.routes[P]),
                F = async () => (
                    (null == _ ? void 0 : _.render404)
                        ? await _.render404(e, t, S, !1)
                    ...
                    l = async (n) => {
                        ...
                        if (
                            (null == u || null == (r = u.value)
                                ? void 0
                                : r.kind) !== h.CachedRouteKind.APP_ROUTE
                        )
                        ...
                        return (
                            ...
                            !u.cacheControl ||
                                ...
                                c.set(
                                    'Cache-Control',
                                    (0, y.getCacheControlHeader)(u.cacheControl)
                                ),
                            await (0, m.sendResponse)(
                                B,
                                Y,
