module.exports = [
    ...
    (e) => {
        ...
        async function A(e) {
            try {
                var t;
                let r = await (0, E.requireAdminForModule)('CHECKOUT'),
                    n = r.companyId;
                if (!n) return y('Empresa não encontrada na sessão.', 401);
                let a = r.id;
                if (!a) return y('Usuário não encontrado na sessão.', 401);
                ...
                if (i)
                    if (s && 'all' !== s) {
                        ...
                        if (!e) return y('Unidade inválida ou inativa.', 404);
                        o = [e.id];
                    } else o = null;
                else {
                    let e = (
                        await w.prisma.adminUnitAccess.findMany({
                            where: { companyId: n, userId: a },
                            select: { unitId: !0 },
                        })
                    ).map((e) => e.unitId);
                    if (0 === e.length) return y('Sem acesso a unidades.', 403);
                    if (s && 'all' !== s) {
                        if (!e.includes(s))
                            return y('Sem acesso a esta unidade.', 403);
                        o = [s];
                    } else o = e;
                }
                let l =
                        o && o.length > 0
                            ? { units: { some: { unitId: { in: o } } } }
                            : {},
                    d = await w.prisma.professional.findMany({
                        where: { companyId: n, ...l },
                        ...
                        select: {
                            ...
                            name: !0,
                            isActive: !0,
                            units: { select: { unitId: !0 } },
                        },
                    }),
                    u = o && 1 === o.length ? o[0] : null;
                return (function (e, t = 200) {
                    ...
                    professionals: d.map((e) => ({
                        id: e.id,
                        name: e.name,
                        isActive: e.isActive,
                        unitId: u ?? e.units?.[0]?.unitId ?? null,
                    })),
                    count: d.length,
                    unitScope: o ? 'filtered' : 'all',
                });
            } catch (e) {
                return y(e?.message ?? 'Erro interno.', 500);
            }
        }
        e.s(['GET', () => A, 'dynamic', 0, 'force-dynamic'], 245019);
        var C = e.i(245019);
        let T = new t.AppRouteRouteModule({
                definition: {
                    kind: r.RouteKind.APP_ROUTE,
                    page: '/api/admin/checkout/professionals/route',
                    pathname: '/api/admin/checkout/professionals',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/admin/checkout/professionals/route.ts',
        ...
        async function b(e, t, n) {
            ...
            let g = '/api/admin/checkout/professionals/route';
            g = g.replace(/\/index$/, '') || '/';
            ...
            let {
                ...
                _ = !!(N.dynamicRoutes[q] || N.routes[I]),
                j = async () => (
            ...
            if (_ && !k) {
                let e = !!N.routes[I],
                    t = N.dynamicRoutes[q];
                if (t && !1 === t.fallback && !e) {
                    if (A.experimental.adapterPath) return await j();
                    throw new x.NoFallbackError();
                }
            }
            let M = null;
            !_ || T.isDev || k || (M = '/index' === (M = I) ? '/' : M);
            let H = !0 === T.isDev || !_,
                D = _ && !H;
            U &&
                O &&
            ...
            try {
                let s = async (e) =>
                    ...
                    l = async (a) => {
                        ...
                        return (
                            (o && _) || c.delete(m.NEXT_CACHE_TAGS_HEADER),
                            !u.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__77c54a7c._.js.map
