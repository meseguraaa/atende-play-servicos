module.exports = [
    ...
    (e) => {
        ...
        async function C(e, t) {
            try {
                var r;
                let e = await (0, y.requireAdminForModule)('SETTINGS');
                if (!e.isOwner) return E('forbidden_owner_only', 403);
                let { unitId: n, exceptionId: a } = await t.params,
                    i = String(n ?? '').trim(),
                    s = String(a ?? '').trim();
                if (!i) return E('unit_id_required', 400);
                if (!s) return E('exception_id_required', 400);
                if (
                    ...
                    return E('unit_not_found', 404);
                let o = await w.prisma.unitDailyAvailability.findFirst({
                    where: { id: s, unitId: i, companyId: e.companyId },
                    select: { id: !0 },
                });
                if (!o) return E('exception_not_found', 404);
                return (
                    await w.prisma.$transaction([
                        w.prisma.unitDailyTimeInterval.deleteMany({
                            where: { dailyAvailabilityId: o.id },
                        }),
                        w.prisma.unitDailyAvailability.delete({
                            where: { id: o.id },
                        }),
                    ]),
                    (r = { id: o.id }),
        ...
        e.s(['DELETE', () => C, 'dynamic', 0, 'force-dynamic'], 128086);
        var I = e.i(128086);
        let A = new t.AppRouteRouteModule({
                definition: {
                    kind: r.RouteKind.APP_ROUTE,
                    page: '/api/admin/settings/units/[unitId]/exceptions/[exceptionId]/route',
                    pathname:
                ...
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/admin/settings/units/[unitId]/exceptions/[exceptionId]/route.ts',
        ...
        async function S(e, t, n) {
            ...
            let g =
                '/api/admin/settings/units/[unitId]/exceptions/[exceptionId]/route';
            g = g.replace(/\/index$/, '') || '/';
            let w = await A.prepare(e, t, {
                srcPage: g,
            ...
            let {
                ...
                k = (0, d.normalizeAppPath)(g),
                j = !!(T.dynamicRoutes[k] || T.routes[P]),
                D = async () => (
                    (null == b ? void 0 : b.render404)
                        ? await b.render404(e, t, I, !1)
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c0460ff4._.js.map
