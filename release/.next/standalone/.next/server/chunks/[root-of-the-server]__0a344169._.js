module.exports = [
    ...
    (e) => {
        ...
        async function E(e, t) {
            try {
                let e = await (0, g.requireAdminForModule)('SETTINGS'),
                    { unitId: r } = await t.params;
                if (!r) return A('unit_id_required', 400);
                if (
                    !(await R.prisma.unit.findFirst({
                        where: { id: r, companyId: e.companyId },
                        select: { id: !0 },
                    }))
                )
                    return A('unit_not_found', 404);
                let n = (
                        await R.prisma.unitWeeklyAvailability.findMany({
                            where: { unitId: r },
                            include: {
                                intervals: {
                                    orderBy: { startTime: 'asc' },
                                    select: { startTime: !0, endTime: !0 },
                                },
                            },
                        })
                    ).reduce((e, t) => ((e[t.weekday] = t), e), {}),
                    a = Array.from({ length: 7 }).map((e, t) => {
                        let r = n[t],
                            a = r?.intervals?.[0] ?? null;
                        return {
                            weekday: t,
        ...
        async function I(e, t) {
            try {
                let r = await (0, g.requireAdminForModule)('SETTINGS');
                if (!r.isOwner) return A('forbidden_owner_only', 403);
                let { unitId: n } = await t.params;
                if (!n) return A('unit_id_required', 400);
                if (
                    !(await R.prisma.unit.findFirst({
                ...
                let i = (function (e) {
                    if (!e || 'object' != typeof e || !Array.isArray(e.weekly))
                        return null;
                    let t = e.weekly,
                        r = [];
                    for (let e of t) {
                        let t = Number(e?.weekday);
                        if (
                            !(
                                'number' == typeof t &&
                                Number.isInteger(t) &&
                ...
                if (!i) return A('invalid_weekly_payload', 400);
                await R.prisma.$transaction(async (e) => {
                    for (let t of i) {
                        let a = await e.unitWeeklyAvailability.upsert({
                            where: {
                                unitId_weekday: {
                                    unitId: n,
                                    weekday: t.weekday,
                        ...
                        (await e.unitWeeklyTimeInterval.deleteMany({
                            where: { weeklyAvailabilityId: a.id },
                        }),
                            t.isActive &&
                                t.startTime &&
                                t.endTime &&
                                (await e.unitWeeklyTimeInterval.create({
                                    data: {
                ...
                let s = (
                    ...
                    l = Array.from({ length: 7 }).map((e, t) => {
                        let r = s[t],
                            n = r?.intervals?.[0] ?? null;
                        return {
                            weekday: t,
                            isActive: !!r && !!r.isActive,
        ...
        e.s(
            ['GET', () => E, 'PUT', () => I, 'dynamic', 0, 'force-dynamic'],
            770266
        );
        var _ = e.i(770266);
        let C = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/admin/settings/units/[unitId]/weekly/route',
                    pathname: '/api/admin/settings/units/[unitId]/weekly',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/admin/settings/units/[unitId]/weekly/route.ts',
        ...
        async function q(e, t, n) {
            ...
            let x = '/api/admin/settings/units/[unitId]/weekly/route';
            x = x.replace(/\/index$/, '') || '/';
            ...
            let {
                ...
                j = (0, o.normalizeAppPath)(x),
                O = !!(I.dynamicRoutes[j] || I.routes[P]),
                M = async () => (
                    ...
                    o = async (a) => {
                        ...
                        if (
                            ...
                            throw Object.defineProperty(
                                Error(
                                    `Invariant: app-route received invalid cache entry ${null == u || null == (o = u.value) ? void 0 : o.kind}`
                                ),
                                '__NEXT_ERROR_CODE',
                                {
                                    value: 'E701',
                        ...
                        let c = (0, y.fromNodeOutgoingHttpHeaders)(
                            u.value.headers
                        );
                        return (
                            ...
                            !u.cacheControl ||
                                ...
                                c.set(
                                    'Cache-Control',
                                    (0, w.getCacheControlHeader)(u.cacheControl)
                                ),
                            await (0, m.sendResponse)(
                                G,
                                W,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0a344169._.js.map
