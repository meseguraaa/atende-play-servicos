module.exports = [
    ...
    (e) => {
        ...
        async function A(e) {
            ...
            let n = await (0, N.verifyAppJwt)(r),
        ...
        function O(e, t) {
            let r = S(t);
            return x((Number.isFinite(e) ? e : 0) * (1 - r / 100));
        }
        async function _(e) {
            let t = e.timeZone ?? 'America/Sao_Paulo',
                ...
                i = e.product,
                a = new Map();
            for (let t of e.discountRows)
                a.set(t.level, S(Number(t.discountPct)));
            function o(e) {
                for (let t of P[e])
                    if (a.has(t)) return { level: t, discountPct: a.get(t) };
            ...
            let s = (function (e) {
                ...
                l = !1;
            if (e.clientBirthday && i.birthdayBenefitEnabled) {
                var u;
                let n,
                    i = T(r, t),
                    a = T(e.clientBirthday, t),
                    o = C(i.year, a.month, a.day),
                    s = I(o, -3),
                    d = I(o, 3);
                ((u = C(i.year, i.month, i.day)),
                    (l = (n = u.getTime()) >= s.getTime() && n <= d.getTime()));
            }
            if (l && i.birthdayBenefitEnabled) {
                let e = o(i.birthdayPriceLevel ?? 'DIAMANTE'),
                    t = O(s, e.discountPct);
                return {
                    unitId: i.unitId,
                    basePrice: s,
            ...
            return {
                ...
                appliedLevel: d.level,
                appliedBecause: d.discountPct > 0 ? 'LEVEL' : 'BASE',
                inBirthdayWindow: !1,
            };
        }
        async function B() {
            return new v.NextResponse(null, { status: 204, headers: b() });
        }
        async function D(e) {
            let t = b();
            try {
                ...
                let i,
                    ...
                    o = a.companyId,
                    s = (function (e) {
                        let t = E(e, 'x-forwarded-proto'),
                            r = E(e, 'x-forwarded-host') || E(e, 'host'),
                            n = String(t ?? '')
                                .split(',')[0]
                                .trim()
                                .toLowerCase(),
                ...
                let u = new URL(e.url),
                    d = (u.searchParams.get('unit') ?? 'all').trim(),
                    c = (u.searchParams.get('q') ?? '').trim(),
                    p = (u.searchParams.get('category') ?? '').trim(),
                    m = (u.searchParams.get('inStock') ?? '').trim(),
                    h = (u.searchParams.get('cursor') ?? '').trim(),
                    f =
                        ((r = u.searchParams.get('limit')),
                        (i = Number(r)),
                    ...
                    g = null;
                if (!y) {
                    let e = await w.prisma.unit.findFirst({
                        where: { id: d, isActive: !0, companyId: o },
                        select: { id: !0 },
                    });
                    if (!e)
                        return v.NextResponse.json(
                ...
                let R =
                    ...
                    b = await w.prisma.product.findMany({
                        where: N,
                        orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
                        take: f + 1,
                        ...(h ? { cursor: { id: h }, skip: 1 } : {}),
                        include: { unit: { select: { id: !0, name: !0 } } },
                    }),
                    P = b.length > f,
                    T = P ? b.slice(0, f) : b,
                    C = 'CLIENT' === a.role ? a.sub : null,
                    I = C
                        ...
                        : null,
                    O = Array.from(new Set(T.map((e) => e.unitId))).filter(
                        Boolean
                    ),
                    B = new Map();
                if (C && O.length > 0)
                    for (let e of await w.prisma.customerLevelState.findMany({
                        where: { companyId: o, userId: C, unitId: { in: O } },
                ...
                let D = T.map((e) => e.id),
                    M = D.length
                        ? await w.prisma.productDiscountByLevel.findMany({
                              where: { companyId: o, productId: { in: D } },
                              select: {
                                  productId: !0,
                                  level: !0,
                                  discountPct: !0,
                ...
                let L = new Date(),
                        ...
                        T.map(async (e) => {
                            let t =
                                    ...
                                    e.pickupDeadlineDays > 0
                                        ...
                                        : 2,
                                r =
                                    'number' == typeof e.stockQuantity
                                        ? e.stockQuantity
                                        : 0,
                                n = B.get(e.unitId) ?? 'BRONZE',
                                i = await _({
                                    product: {
                                        id: e.id,
                                        price: e.price,
                                        ...
                                        birthdayBenefitEnabled:
                                            !!e.birthdayBenefitEnabled,
                                        birthdayPriceLevel:
                                            e.birthdayPriceLevel ?? null,
                                    },
                                    clientBirthday: I?.birthday ?? null,
                                    effectiveLevel: n,
                                    timeZone: 'America/Sao_Paulo',
                                    now: L,
                                    discountRows: k.get(e.id) ?? [],
                                }),
                                a = Number(i.basePrice),
                                o = Number(i.finalPrice),
                                l = S(Number(i.discountPct)),
                                u =
                                    Number.isFinite(a) &&
                                    Number.isFinite(o) &&
                                    a > 0 &&
                                    o < a,
                                d = u ? x(Math.max(0, a - o)) : 0,
                                c =
                                    'BIRTHDAY' === i.appliedBecause
                                        ? {
                                              type: 'BIRTHDAY',
                                              label: '🎂 Aniversário',
                                          }
                                        : 'LEVEL' === i.appliedBecause && u
                                          ? {
                                                type: 'LEVEL',
                            ...
                            return {
                                ...
                                pricing: {
                                    customerLevel: n,
                                    appliedLevel: i.appliedLevel,
                                    appliedBecause: i.appliedBecause,
                                    inBirthdayWindow: i.inBirthdayWindow,
                                    discountPct: l,
                                },
                                badge: c,
                            };
                    ...
                    F = P ? (U[U.length - 1]?.id ?? null) : null,
                    H = v.NextResponse.json(
                        {
                            ok: !0,
                            items: U,
                            products: U,
                ...
                return (H.headers.set('x-company-id', o), H);
            } catch (r) {
                ...
                return (
                    console.error('[mobile products] error:', r),
                    v.NextResponse.json(
                        { error: 'server_error' },
        ...
        e.s(
            ['GET', () => D, 'OPTIONS', () => B, 'dynamic', 0, 'force-dynamic'],
            560040
        );
        var M = e.i(560040);
        let k = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/mobile/products/route',
                    pathname: '/api/mobile/products',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/mobile/products/route.ts',
        ...
        async function $(e, t, n) {
            ...
            let v = '/api/mobile/products/route';
            v = v.replace(/\/index$/, '') || '/';
            let w = await k.prepare(e, t, {
                srcPage: v,
            ...
            let {
                ...
                D = !!(T.dynamicRoutes[B] || T.routes[x]),
                M = async () => (
                    ...
                    l = async (i) => {
                        ...
                        if (
                            (null == d || null == (a = d.value)
                                ? void 0
                                : a.kind) !== R.CachedRouteKind.APP_ROUTE
                        )
