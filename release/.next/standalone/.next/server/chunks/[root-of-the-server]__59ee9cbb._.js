module.exports = [
    ...
    (e) => {
        ...
        function b(e) {
            return (String(e ?? '').trim(), 'HAS_ACTIVE_PLAN');
        }
        function T(e) {
            let t = String(e ?? '').trim();
            return A.includes(t) ? t : 'BRONZE';
        }
        function U(e, t = 0) {
        ...
        async function P(e) {
            let t = await (0, I.requireAdminForModule)('CLIENT_LEVELS'),
                r = await k(t);
            if (!r)
                return {
                    ok: !1,
            ...
            let a = await (0, x.cookies)(),
                n =
                    String(a.get('admin_unit_context')?.value ?? '').trim() ||
                    'all',
                i = !!t?.canSeeAllUnits,
        ...
        async function O(e) {
            try {
                let { searchParams: t } = new URL(e.url),
                    r = t.get('unitId'),
                    a = await P({ requestedUnitId: r });
                if (!a.ok) return N(a.error, a.status);
                let n = await E.prisma.customerLevelRule.findMany({
                    where: { companyId: a.companyId, unitId: a.activeUnitId },
                ...
                return S({
                    scope: {
                        ...
                        canSeeAllUnits: a.canSeeAllUnits,
                        unitCookie: a.unitCookie,
                    },
                    units: a.units,
                    activeUnitId: a.activeUnitId,
                    levels: A,
                    ruleTypes: C,
                    rules: n.map((e) => ({
                        id: e.id,
        ...
        async function q(e) {
            try {
                let t = await e.formData(),
                    r = String(_(t, 'intent') ?? '').trim(),
                    a = String(_(t, 'unitId') ?? '').trim(),
                    n = await P({ requestedUnitId: a });
                if (!n.ok) return N(n.error, n.status);
                let i = n.activeUnitId;
                if ('create' === r) {
                    if (
                        (await E.prisma.customerLevelRule.count({
                            where: { companyId: n.companyId, unitId: i },
                        })) > 0
                    )
                        return N(
                    ...
                    let e = b(_(t, 'type')),
                        r = T(_(t, 'targetLevel')),
                        a = U(_(t, 'priority'), 100),
                        o = await E.prisma.customerLevelRule.create({
                            data: {
                    ...
                    return S({ saved: !0, intent: 'create', ruleId: o.id });
                }
                if ('update' === r) {
                    let e = String(_(t, 'ruleId') ?? '').trim();
                    if (!e) return N('ruleId é obrigatório.', 400);
                    let r = b(_(t, 'type')),
                        a = T(_(t, 'targetLevel')),
                        o = U(_(t, 'priority'), 100);
                    return (
                        await E.prisma.customerLevelRule.updateMany({
                            where: { id: e, companyId: n.companyId, unitId: i },
                            data: { type: r, targetLevel: a, priority: o },
                        }),
                        S({ saved: !0, intent: 'update', ruleId: e })
                    );
                }
                if ('delete' === r) {
                    let e = String(_(t, 'ruleId') ?? '').trim();
                    if (!e) return N('ruleId é obrigatório.', 400);
                    return (
                        await E.prisma.customerLevelRule.deleteMany({
                            where: { id: e, companyId: n.companyId, unitId: i },
                        }),
                        S({ saved: !0, intent: 'delete', ruleId: e })
                    );
                }
                return N('Intent inválida. Use create, update ou delete.', 400);
            } catch (e) {
                if ((0, w.isRedirectError)(e)) throw e;
                return N(
                    'string' == typeof e?.message
        ...
        e.s(
            ['GET', () => O, 'POST', () => q, 'dynamic', 0, 'force-dynamic'],
            312615
        );
        var L = e.i(312615);
        let M = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/admin/client-levels/rules/route',
                    pathname: '/api/admin/client-levels/rules',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/admin/client-levels/rules/route.ts',
        ...
        async function B(e, t, a) {
            ...
            let R = '/api/admin/client-levels/rules/route';
            R = R.replace(/\/index$/, '') || '/';
            let x = await M.prepare(e, t, {
                srcPage: R,
            ...
            let {
                ...
                O = !!(S.dynamicRoutes[P] || S.routes[T]),
                q = async () => (
            ...
            try {
                let o = async (e) =>
                    ...
                    l = async (n) => {
                        ...
                        return (
                            (s && O) || c.delete(y.NEXT_CACHE_TAGS_HEADER),
                            !u.cacheControl ||
                                t.getHeader('Cache-Control') ||
                                c.get('Cache-Control') ||
                                c.set(
                                    'Cache-Control',
                                    (0, g.getCacheControlHeader)(u.cacheControl)
                                ),
                            await (0, m.sendResponse)(
                                K,
                                V,
