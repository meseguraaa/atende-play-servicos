module.exports = [
    ...
    (e) => {
        ...
        function E(e, t) {
            let r = t.toLowerCase();
            for (let [t, i] of e.headers.entries())
                if (t.toLowerCase() === r) {
                    let e = String(i ?? '').trim();
        ...
        function b(e, t) {
            ...
            let i = r.replace(/\\/g, '/');
            return (
                i.startsWith('public/') && (i = i.slice(6)),
                i.startsWith('/') || (i = `/${i}`),
                `${e}${i}`.replace(/([^:]\/)\/+/g, '$1')
            );
        }
        ...
        async function T(e) {
            let t = E(e, 'authorization') || '',
                r = t.startsWith('Bearer ') ? t.slice(7).trim() : '';
            if (!r) throw Error('missing_token');
            let i = await (0, g.verifyAppJwt)(r),
                n = 'string' == typeof i?.sub ? String(i.sub).trim() : '';
            ...
            let a =
                'string' == typeof i?.companyId
                    ...
                    : '';
            if (!a) {
                let t = E(e, 'x-company-id');
                t && (a = t);
            }
            if (!a) throw Error('missing_company_id');
            if (
                !(await R.prisma.companyMember.findFirst({
                    where: { userId: n, companyId: a, isActive: !0 },
                    select: { id: !0 },
                }))
            )
                throw Error('forbidden_company');
            return { ...i, sub: n, companyId: a };
        }
        let C = {
        ...
        async function k(e) {
            if (!e.unitId) return 'BRONZE';
            let t = await R.prisma.customerLevelState.findFirst({
                where: {
            ...
            return O(t?.levelCurrent) ?? 'BRONZE';
        }
        async function M(e) {
            let t = e.timeZone ?? 'America/Sao_Paulo',
                r = e.now ?? new Date(),
                i = e.effectiveLevel ?? 'BRONZE',
                [n, a] = await Promise.all([
                    R.prisma.product.findFirst({
                        where: {
                            id: e.productId,
                            isActive: !0,
                            companyId: e.companyId,
                    ...
                    e.clientId
                        ? R.prisma.user.findFirst({
                              where: { id: e.clientId },
                              select: { id: !0, birthday: !0 },
                          })
                        : Promise.resolve(null),
                ]);
            if (!n) throw Error('Produto não encontrado.');
            let o = await R.prisma.productDiscountByLevel.findMany({
                    where: { productId: n.id, companyId: e.companyId },
                    select: { level: !0, discountPct: !0 },
                }),
                s = new Map();
            for (let e of o) {
                let t = O(e.level);
                t && s.set(t, U(A(e.discountPct)));
            }
            function d(e) {
                for (let t of C[e])
            ...
            let u = A(n.price),
                l = !1;
            if (a?.birthday && n.birthdayBenefitEnabled) {
                var c;
                let e,
                    i = _(r, t),
                    n = _(a.birthday, t),
                    o = S(i.year, n.month, n.day),
                    s = x(o, -3),
                    d = x(o, 3);
                ((c = S(i.year, i.month, i.day)),
                    (l = (e = c.getTime()) >= s.getTime() && e <= d.getTime()));
            }
            if (l && n.birthdayBenefitEnabled) {
                let e = d(O(n.birthdayPriceLevel) ?? 'DIAMANTE'),
                    t = D(u, e.discountPct);
                return {
                    unitId: n.unitId,
                    basePrice: u,
            ...
            return {
                ...
                appliedBecause: p.discountPct > 0 ? 'LEVEL' : 'BASE',
                inBirthdayWindow: !1,
            };
        }
        ...
        async function F(e, t, r) {
            return await e.orderItem.updateMany({ where: t, data: r });
        }
        function L(e) {
            let t = Number(e ?? 0);
        ...
        async function j() {
            return new w.NextResponse(null, { status: 204, headers: P() });
        }
        async function q(e) {
            let t = P();
            try {
                ...
                let s = await R.prisma.$transaction(async (e) => {
                    let t = r.sub,
                        n = await e.product.findFirst({
                    ...
                    if (!n) throw Error('Produto não encontrado ou inativo.');
                    if (!n.unitId)
                        throw Error(
                            'Produto sem unidade vinculada (unitId). Não é possível criar o pedido.'
                        );
                    let s = Number(n.stockQuantity ?? 0);
                    ...
                    if (
                        u.unitId &&
                        n.unitId &&
                        String(u.unitId) !== String(n.unitId)
                    )
                        throw Error('Produto inválido para esta unidade.');
                    let l =
                            'number' == typeof n.pickupDeadlineDays &&
                            Number.isFinite(n.pickupDeadlineDays) &&
                            n.pickupDeadlineDays > 0
                                ? n.pickupDeadlineDays
                                : 2,
                        c = new Date();
                    c.setDate(c.getDate() + l);
                    let p = new N.Prisma.Decimal(u.finalPrice),
                        m = p.mul(o),
                        f = new Date(),
                        h = await e.order.findFirst({
                            where: {
                                companyId: i,
                                clientId: r.sub,
                        ...
                        y =
                            h?.reservedUntil && h.reservedUntil > c
                                ? h.reservedUntil
                                : c;
                    if (!h) {
                        let t = await e.order.create({
                            data: {
                                companyId: i,
                                clientId: r.sub,
                                appointmentId: null,
                        ...
                        return (
                            ...
                            { orderId: t.id, reservedUntil: t.reservedUntil }
                        );
                    }
                    let I = h.items?.[0] ?? null;
                    if (I) {
                        let t = Number(I.quantity ?? 0) + o;
                        if (s < t)
                            throw Error('Quantidade indisponível no estoque.');
                        let r = p.mul(t),
                            n = r.sub(I.totalPrice);
                        return (
                            await F(
                                e,
                                { id: I.id, orderId: h.id, companyId: i },
                                { quantity: t, unitPrice: p, totalPrice: r }
                            ),
                            await e.order.updateMany({
                                where: { id: h.id, companyId: i },
                                data: {
                                    reservedUntil: y,
                                    totalAmount: h.totalAmount.add(n),
                                },
                            }),
                            { orderId: h.id, reservedUntil: y }
                        );
                    }
                    return (
                        await B(
                        ...
                        await e.order.updateMany({
                            ...
                            data: {
                                reservedUntil: y,
                                totalAmount: h.totalAmount.add(m),
                            },
                        }),
                        { orderId: h.id, reservedUntil: y }
                    );
                ...
                if (
                    ...
                    e.includes('Quantidade indisponível') ||
                    e.includes('Produto sem unidade') ||
                    e.includes('Produto inválido')
                )
                    return w.NextResponse.json(
                        { error: e },
                        { status: 400, headers: t }
                ...
                return (
                    console.error('[mobile orders POST] error:', r),
                    w.NextResponse.json(
                        { error: 'server_error' },
        ...
        async function $(e) {
            ...
            try {
                ...
                let o = (function (e) {
                        let t = String(process.env.APP_PUBLIC_BASE_URL ?? '')
                            .trim()
                            .replace(/\/+$/, '');
                        if (t) return t;
                        let r = E(e, 'x-forwarded-host') || E(e, 'host') || '',
                            i =
                                E(e, 'x-forwarded-proto') ||
                    ...
                    s = new URL(e.url),
                    d = (s.searchParams.get('view') ?? '').trim(),
                    u = (s.searchParams.get('status') ?? '').trim(),
                    l = (s.searchParams.get('cursor') ?? '').trim(),
                    c =
                        ((r = s.searchParams.get('limit')),
                        (i = Number(r)),
                        !Number.isFinite(i) || i <= 0
                            ? 20
                            : Math.min(50, Math.floor(i))),
                    p =
                        u ||
                        ('bag' === d
                            ? 'PENDING_CHECKIN'
                            : 'history' === d
                              ? 'HISTORY'
                              : ''),
                    m = { companyId: a, clientId: n.sub };
                (p && 'HISTORY' !== p && (m.status = p),
                    'HISTORY' === p &&
                        (m.status = { in: ['COMPLETED', 'CANCELED'] }));
                let f = await R.prisma.order.findMany({
                        where: m,
                        orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
                        take: c + 1,
                        ...(l ? { cursor: { id: l }, skip: 1 } : {}),
                        select: {
                            id: !0,
                            status: !0,
                            reservedUntil: !0,
                            ...
                            let t = await k({
                                ...
                                r = await Promise.all(
                                    (e.items ?? []).map(async (e) => {
                                        ...
                                        let d = L(Number(s.basePrice)),
                                            u = L(Number(s.finalPrice)),
                                            l = U(Number(s.discountPct ?? 0)),
                                            c = Number.isFinite(u) ? u : i,
                                            p = L(c * r),
                                            m =
                                                Number.isFinite(d) &&
                                                Number.isFinite(c) &&
                                ...
                                i = L(
                                    r.reduce((e, t) => e + L(t?.totalPrice), 0)
                                );
                            return {
                                id: e.id,
                                status: e.status,
                ...
                return (
                    console.error('[mobile orders GET] error:', r),
                    w.NextResponse.json(
                        { error: 'server_error' },
                        { status: 500, headers: t }
                    )
        ...
        let K = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/mobile/orders/route',
                    pathname: '/api/mobile/orders',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/mobile/orders/route.ts',
        ...
        async function V(e, t, i) {
            ...
            let w = '/api/mobile/orders/route';
            w = w.replace(/\/index$/, '') || '/';
            let N = await K.prepare(e, t, {
                srcPage: w,
            ...
            let {
                ...
                D = !!(A.dynamicRoutes[U] || A.routes[S]),
                k = async () => (
                    (null == T ? void 0 : T.render404)
                        ? await T.render404(e, t, E, !1)
            ...
            if (D && !b) {
                let e = !!A.routes[S],
                    t = A.dynamicRoutes[U];
                if (t && !1 === t.fallback && !e) {
                    if (P.experimental.adapterPath) return await k();
                    throw new I.NoFallbackError();
                }
            }
            let M = null;
            !D || K.isDev || b || (M = '/index' === (M = S) ? '/' : M);
            let B = !0 === K.isDev || !D,
                F = D && !B;
            O &&
                x &&
...
//# sourceMappingURL=c2dd8_next_dist_esm_build_templates_app-route_f7b221bc.js.map
