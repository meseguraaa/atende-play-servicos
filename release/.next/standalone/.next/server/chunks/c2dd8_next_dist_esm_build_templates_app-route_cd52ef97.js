    ...
    (e) => {
        ...
        async function A(e) {
            let t = e.headers.get('authorization') || '',
                i = t.startsWith('Bearer ') ? t.slice(7).trim() : '';
            if (!i) throw Error('missing_token');
            let r = await (0, b.verifyAppJwt)(i),
                n =
                    'string' == typeof r?.companyId
                        ? String(r.companyId).trim()
                        : '';
            if (!n) throw Error('missing_company_id');
            return { ...r, companyId: n };
        }
        async function R() {
            return new w.NextResponse(null, { status: 204, headers: y() });
        }
        function E(e) {
        ...
        function I(e) {
            let t = String(e ?? '').trim();
            if (!t) return '';
            let i = t.match(/^(\d{1,2}):(\d{2})/);
            return i ? `${E(Number(i[1]))}:${i[2]}` : '';
        }
        function C(e) {
            let t = String(e ?? '').match(/^(\d{2}):(\d{2})$/);
            return t ? 60 * Number(t[1]) + Number(t[2]) : 1 / 0;
        }
        function M(e, t = 30) {
            let i = Number(e);
            return !Number.isFinite(i) || i <= 0
                ? t
                : Math.max(1, Math.round(i));
        }
        function _(e) {
            return e
                .map((e) => ({
                ...
                .filter(
                    (e) =>
                        Number.isFinite(e._s) &&
                        Number.isFinite(e._e) &&
                        e._e > e._s
                )
                .sort((e, t) => e._s - t._s);
        }
        async function x(e) {
            ...
            try {
                ...
                let n,
                    a = await A(e),
                    s = a.companyId;
                if (a.role && 'CLIENT' !== a.role)
                    return w.NextResponse.json(
                        { ok: !1, error: 'Sem permissão' },
                        { status: 403, headers: t }
                    );
                let { searchParams: o } = new URL(e.url),
                    l = String(
                        o.get('professionalId') ?? o.get('barberId') ?? ''
                    ).trim(),
                    u = String(o.get('unitId') ?? '').trim(),
                    d = String(o.get('dateISO') ?? '').trim(),
                    m = String(o.get('appointmentId') ?? '').trim(),
                    c = String(o.get('serviceId') ?? '').trim(),
                    p = (function (e) {
                        if (!e) return null;
                        let t = Number(e);
                        return !Number.isFinite(t) || t <= 0
                            ? null
                            : Math.round(t);
                    })(o.get('serviceDurationInMinutes'));
                if (!l || !u || !d)
                    return w.NextResponse.json(
                        {
                            ok: !1,
                ...
                if (Number.isNaN(f.getTime()))
                    return w.NextResponse.json(
                        { ok: !1, error: 'dateISO inválido' },
                        { status: 400, headers: t }
                    );
                if (
                    !(await N.prisma.unit.findFirst({
                        where: { id: u, companyId: s, isActive: !0 },
                        select: { id: !0 },
                    }))
                )
                    return w.NextResponse.json(
                ...
                if (
                    ...
                    return w.NextResponse.json(
                        { ok: !1, error: 'Profissional inválido' },
                        { status: 404, headers: t }
                    );
                let h = p;
                if (!h && c) {
                    let e = await N.prisma.service.findFirst({
                        where: { id: c, companyId: s, isActive: !0 },
                        select: { durationMinutes: !0 },
                    });
                    if (!e)
                        return w.NextResponse.json(
                    ...
                    h = Math.max(1, Number(e?.durationMinutes ?? 0)) || 30;
                }
                if (!h)
                    return w.NextResponse.json(
                        {
                ...
                let T =
                        ...
                        }[String(n)] ?? f.getUTCDay()),
                    {
                        ...
                        if (!n || !a || !s) {
                            ...
                            return (
                                ...
                                { startUtc: e, endUtc: i, dateKey: null }
                            );
                        }
                        let o = new Date(Date.UTC(n, a - 1, s, 3, 0, 0, 0)),
                            l = new Date(
                                Date.UTC(n, a - 1, s + 1, 2, 59, 59, 999)
                            );
                        return {
                            startUtc: o,
                            endUtc: l,
                    ...
                    y = await N.prisma.professionalWeeklyAvailability.findFirst(
                        {
                            where: {
                                companyId: s,
                                professionalId: l,
                    ...
                    R = (y?.intervals ?? [])
                        ...
                        .filter((e) => e.startTime && e.endTime),
                    x = await N.prisma.professionalDailyAvailability.findFirst({
                        where: {
                            companyId: s,
                            professionalId: l,
                            unitId: u,
                ...
                x?.type === 'DAY_OFF'
                    ...
                    : x?.type === 'CUSTOM'
                      ...
                      : R.length
                        ? ((D = 'WEEKLY'), (F = R))
                        : ((D = 'NONE'), (F = []));
                let O = await N.prisma.unitWeeklyAvailability.findFirst({
                        where: {
                            companyId: s,
                            unitId: u,
                            weekday: T,
                    ...
                    P = (O?.intervals ?? [])
                        ...
                        .filter((e) => e.startTime && e.endTime),
                    k = await N.prisma.unitDailyAvailability.findFirst({
                        where: {
                            companyId: s,
                            unitId: u,
                            date: { gte: v, lte: g },
                ...
                let H = (function (e, t) {
                    let i = _(e),
                        r = _(t),
                        n = [];
                    if (!i.length || !r.length) return n;
                    let a = 0,
                        s = 0;
                    for (; a < i.length && s < r.length; ) {
                        let e = i[a],
                            t = r[s],
                            o = Math.max(e._s, t._s),
                            l = Math.min(e._e, t._e);
                        (l > o && n.push({ startTime: S(o), endTime: S(l) }),
                            e._e < t._e ? a++ : s++);
                    }
                    return n;
                })(F, j);
                    ...
                    return w.NextResponse.json(
                        {
                            ...
                            meta: {
                                ...
                                professionalId: l,
                                serviceId: c || null,
                                serviceDurationInMinutes: h,
                                slotIntervalInMinutes: 30,
                                appointmentId: m || null,
                                weekday: T,
                                dateKeySaoPaulo: b,
                                professionalAvailabilitySource: D,
                                unitAvailabilitySource: U,
                                hasAvailability: !1,
                            },
                        },
                ...
                for (let e of H)
                    for (let t of (function (e) {
                        let t = C(I(e.startTime)),
                            ...
                            r =
                                ...
                                e.durationMin > 0
                                    ...
                                    : 30,
                            n =
                                Number.isFinite(e.stepMin) && e.stepMin > 0
                                    ? e.stepMin
                                    : 30;
                        if (
                            !Number.isFinite(t) ||
                            ...
                            return [];
                        let a = i - r;
                        if (!Number.isFinite(a) || a < t) return [];
                        let s = [];
                        for (let e = t; e <= a; e += n) s.push(S(e));
                        return s;
                    })({
                        startTime: e.startTime,
                        endTime: e.endTime,
                        ...
                        $.add(t);
                let L = Array.from($).sort((e, t) => C(e) - C(t)),
                    q = (
                        (await N.prisma.appointment.findMany({
                            where: {
                                companyId: s,
                                unitId: u,
                                professionalId: l,
                                status: { not: 'CANCELED' },
                                scheduleAt: { gte: v, lte: g },
                                ...(m ? { id: { not: m } } : {}),
                            },
                            select: {
                                id: !0,
                        ...
                        .map((e) => {
                            ...
                            let i,
                                ...
                                u = M(e?.service?.durationMinutes ?? 30, 30),
                                d = l + u;
                ...
                if (
                    ...
                    let e = await N.prisma.appointment.findFirst({
                        where: {
                            id: m,
                            companyId: s,
                            clientId: a.sub,
                    ...
                    if (
                        e &&
                        String(e.unitId ?? '') === u &&
                        String(e.professionalId ?? '') === l &&
                        ((i = new Date(e.scheduleAt)),
                        i.getFullYear() === f.getFullYear() &&
                            i.getMonth() === f.getMonth() &&
                            i.getDate() === f.getDate())
                    ) {
                        let t =
                            ((r = new Date(e.scheduleAt)),
                            `${E(r.getHours())}:${E(r.getMinutes())}`);
                        t && !L.includes(t) && (L = [t, ...L]);
                    }
                }
                return w.NextResponse.json(
                    {
                        ...
                        meta: {
                            ...
                            professionalAvailabilitySource: D,
                            unitAvailabilitySource: U,
                            effectiveIntervals: H,
                            busyCount: q.length,
                            hasAvailability: L.length > 0,
                        },
                    },
                    { status: 200, headers: t }
                );
            } catch (i) {
                let e = String(i?.message ?? 'Não autorizado').toLowerCase();
                if (
                ...
                return (
                    console.error('[api/mobile/availability] error:', i),
                    w.NextResponse.json(
                        { ok: !1, error: 'Erro ao buscar disponibilidade' },
        ...
        e.s(
            ['GET', () => x, 'OPTIONS', () => R, 'runtime', 0, 'nodejs'],
            222266
        );
        var D = e.i(222266);
        let F = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/mobile/availability/route',
                    pathname: '/api/mobile/availability',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/mobile/availability/route.ts',
        ...
        async function j(e, t, r) {
            ...
            let w = '/api/mobile/availability/route';
            w = w.replace(/\/index$/, '') || '/';
            let N = await F.prepare(e, t, {
                srcPage: w,
            ...
            let {
                ...
                P = !!(S.dynamicRoutes[O] || S.routes[_]),
                k = async () => (
                    (null == I ? void 0 : I.render404)
                        ? await I.render404(e, t, R, !1)
            ...
            if (P && !E) {
                let e = !!S.routes[_],
                    t = S.dynamicRoutes[O];
                if (t && !1 === t.fallback && !e) {
                    if (A.experimental.adapterPath) return await k();
                    throw new v.NoFallbackError();
                }
            }
            let U = null;
            !P || F.isDev || E || (U = '/index' === (U = _) ? '/' : U);
            let j = !0 === F.isDev || !P,
                H = P && !j;
            D &&
                x &&
            ...
            let $ = e.method || 'GET',
                ...
                K = {
                    ...
                    renderOpts: {
                        ...
                        onInstrumentationRequestError: (t, i, r) =>
                            F.onRequestError(e, t, r, I),
                    },
                    sharedContext: { buildId: b },
                },
                Y = new u.NodeNextRequest(e),
                B = new u.NodeNextResponse(t),
                W = d.NextRequestAdapter.fromNodeNextRequest(
                    Y,
                    (0, d.signalFromNodeResponse)(t)
                );
            try {
                let s = async (e) =>
                        F.handle(W, K).finally(() => {
                            if (!e) return;
                            e.setAttributes({
                    ...
                    l = async (n) => {
                        var a, l;
                        let u = async ({ previousCacheEntry: i }) => {
                                try {
                                    if (!o && C && M && !i)
                                        return (
                                            (t.statusCode = 404),
                                        ...
                                        );
                                    {
                                        ...
                                        (u && (t[T.NEXT_CACHE_TAGS_HEADER] = u),
                                            !t['content-type'] &&
                                                e.type &&
                        ...
                        if (
                            (null == d || null == (a = d.value)
                                ? void 0
                                : a.kind) !== g.CachedRouteKind.APP_ROUTE
                        )
                        ...
                        let m = (0, f.fromNodeOutgoingHttpHeaders)(
                            d.value.headers
                        );
                        return (
                            (o && P) || m.delete(T.NEXT_CACHE_TAGS_HEADER),
                            !d.cacheControl ||
