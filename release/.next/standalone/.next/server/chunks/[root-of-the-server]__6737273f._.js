module.exports = [
    ...
    (e) => {
        ...
        function C(e, t = 200) {
            return E.NextResponse.json({ ok: !0, data: e }, { status: t });
        }
        async function N(e, t) {
            try {
                let e = await (0, y.requireAdminForModule)('CHECKOUT'),
                    r = e.companyId;
                if (!r) return A('Empresa não encontrada na sessão.', 401);
                let n = e.id;
                if (!n) return A('Usuário não encontrado na sessão.', 401);
                let a = e.canSeeAllUnits,
                    { orderId: i } = await t.params,
                    o = g(i);
                if (!o) return A('orderId é obrigatório.', 400);
                let s = await w.prisma.order.findFirst({
                    where: { id: o, companyId: r },
                    select: {
                        id: !0,
                        unitId: !0,
                        status: !0,
                        inventoryRevertedAt: !0,
                        items: { select: { productId: !0, quantity: !0 } },
                    },
                });
                if (!s) return A('Pedido não encontrado.', 404);
                if (
                    !a &&
                    !(await w.prisma.adminUnitAccess.findFirst({
                        where: { companyId: r, userId: n, unitId: s.unitId },
                        select: { id: !0 },
                    }))
                )
                    return A('Sem acesso a esta unidade.', 403);
                if ('CANCELED' === s.status && s.inventoryRevertedAt)
                    return C({
                        orderId: s.id,
                        status: 'CANCELED',
                ...
                if (
                    'PENDING' !== s.status &&
                    'PENDING_CHECKIN' !== s.status &&
                    'CANCELED' !== s.status
                )
                    return A(
                        `N\xe3o \xe9 poss\xedvel cancelar pedido com status "${s.status}".`,
                        400
                    );
                let d = new Date(),
                    l = await w.prisma.$transaction(async (e) => {
                        if (!s.inventoryRevertedAt) {
                            let t = new Map();
                            ...
                            for (let [n, a] of t.entries())
                                await e.product.updateMany({
                                    where: { id: n, companyId: r },
                                    data: { stockQuantity: { increment: a } },
                                });
                        }
                        let t = await e.order.updateMany({
                            where: { id: s.id, companyId: r },
                            data: {
                                status: 'CANCELED',
                                expiredAt: d,
                                inventoryRevertedAt: s.inventoryRevertedAt ?? d,
                            },
                        });
                        if (0 === t.count)
                            throw Error(
                        ...
                        let n = await e.order.findFirst({
                            where: { id: s.id, companyId: r },
                            select: { id: !0, inventoryRevertedAt: !0 },
                        });
                        if (!n)
                            throw Error(
                ...
                return C({
                    orderId: l.id,
                    status: 'CANCELED',
                    canceledAt: d.toISOString(),
                    inventoryRevertedAt: l.inventoryRevertedAt
                        ? l.inventoryRevertedAt.toISOString()
                        : null,
                });
            } catch (e) {
        ...
        let S = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/admin/checkout/orders/[orderId]/cancel/route',
                    pathname: '/api/admin/checkout/orders/[orderId]/cancel',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/admin/checkout/orders/[orderId]/cancel/route.ts',
        ...
        async function k(e, t, n) {
            ...
            let E = '/api/admin/checkout/orders/[orderId]/cancel/route';
            E = E.replace(/\/index$/, '') || '/';
            let w = await S.prepare(e, t, {
                srcPage: E,
            ...
            let {
                ...
                D = (0, d.normalizeAppPath)(E),
                M = !!(I.dynamicRoutes[D] || I.routes[b]),
                _ = async () => (
            ...
            try {
                let o = async (e) =>
                    ...
                    d = async (a) => {
                        ...
                        return (
                            (s && M) || c.delete(f.NEXT_CACHE_TAGS_HEADER),
                            !u.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__6737273f._.js.map
