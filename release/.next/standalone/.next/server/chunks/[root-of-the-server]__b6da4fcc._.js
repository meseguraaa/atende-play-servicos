module.exports = [
    ...
    (e) => {
        ...
        async function C(e, t) {
            let r = await (0, y.requireAdminForModuleApi)('FINANCE');
            if (r instanceof v.NextResponse) return r;
            let n = r?.companyId ?? null;
            if (!n) return E('missing_company', 403);
            let a = r?.userId ?? null,
                { expenseId: i } = await t.params,
                s = String(i || '').trim();
            if (!s) return E('expense_id_required', 400);
            let o = await w.prisma.expense.findFirst({
            ...
            if (!o) return E('expense_not_found', 404);
            if (!r?.canSeeAllUnits)
                if (a) {
                    let e = await w.prisma.adminUnitAccess.findFirst({
                        where: { companyId: n, userId: a, unitId: o.unitId },
                        select: { id: !0 },
                    });
                    if (!e?.id) {
                        let e = r?.unitId ?? null;
                        if (!e) return E('missing_admin_unit', 403);
                        if (o.unitId !== e) return E('forbidden_unit', 403);
                    }
                } else {
                    let e = r?.unitId ?? null;
                    if (!e) return E('missing_admin_unit', 403);
            ...
            try {
                var d;
                return (
                    (d = await w.prisma.$transaction(async (e) => {
                        if (!o.isRecurring)
                            return (
                                await e.expense.delete({ where: { id: o.id } }),
                                { deleted: !0, deletedCount: 1, mode: 'single' }
                            );
                        let t = await e.expense.deleteMany({
                            where: {
                                ...
                                amount: o.amount,
                                dueDate: { gte: o.dueDate },
                            },
                        });
                        return {
                            deleted: t.count > 0,
                            deletedCount: t.count,
                            mode: 'series',
                        };
                    })),
                    v.NextResponse.json({ ok: !0, data: d }, void 0)
                );
            } catch {
                return E('internal_error', 500);
            }
        }
        e.s(['DELETE', () => C], 647322);
        var A = e.i(647322);
        let I = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/admin/finance/expenses/[expenseId]/route',
                    pathname: '/api/admin/finance/expenses/[expenseId]',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/admin/finance/expenses/[expenseId]/route.ts',
        ...
        async function P(e, t, n) {
            ...
            let v = '/api/admin/finance/expenses/[expenseId]/route';
            v = v.replace(/\/index$/, '') || '/';
            let w = await I.prepare(e, t, {
                srcPage: v,
            ...
            let {
                ...
                O = (0, d.normalizeAppPath)(v),
                j = !!(N.dynamicRoutes[O] || N.routes[S]),
                M = async () => (
            ...
            try {
                let s = async (e) =>
                    ...
                    d = async (a) => {
                        ...
                        return (
                            (o && j) || p.delete(m.NEXT_CACHE_TAGS_HEADER),
                            !u.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b6da4fcc._.js.map
