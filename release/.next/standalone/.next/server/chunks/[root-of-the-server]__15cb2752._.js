module.exports = [
    ...
    (e) => {
        ...
        async function I(e) {
            ...
            if (t) return t;
            let r = await (0, R.cookies)(),
                n = r.get(A)?.value;
            if (n) return n;
            let a = String(e?.userId ?? '').trim();
            if (a) {
                let e = await y.prisma.companyMember.findFirst({
                    where: { userId: a, isActive: !0 },
            ...
            throw Error(
                `companyId ausente (session.companyId, cookie "${A}" e sem fallback por membership).`
            );
        }
        async function N() {
            let e = await (0, R.cookies)(),
        ...
        async function T(e, t) {
            try {
                var r;
                let n = await (0, E.requireAdminForModule)('CLIENTS'),
                    a = await I(n),
                    { id: i } = await t.params,
                    o = b(i);
                if (!o) return C('ID do cliente ausente.');
                let s = await e.json().catch(() => null);
                if (!s) return C('Body inválido.');
                let l = b(s.name),
                    u = b(s.email).toLowerCase(),
                    d = b(s.phone),
                    c = b(s.birthday);
                if (!l) return C('Informe o nome do cliente.');
                if (!u) return C('Informe o e-mail do cliente.');
                if (!d) return C('Informe o telefone do cliente.');
                if (String(d ?? '').replace(/\D/g, '').length < 10)
                ...
                let p = (function (e) {
                    ...
                    if (t.includes('/')) {
                        ...
                        let r = Number(e[1]),
                            n = Number(e[2]),
                            a = Number(e[3]);
                        if (!a || n < 1 || n > 12 || r < 1 || r > 31)
                            return null;
                        let i = new Date(a, n - 1, r);
                        return i.getFullYear() !== a ||
                            i.getMonth() !== n - 1 ||
                            i.getDate() !== r
                            ? null
                            : i;
                    ...
                    if (t.includes('-')) {
                        ...
                        let r = Number(e[1]),
                            n = Number(e[2]),
                            a = Number(e[3]);
                        if (!r || n < 1 || n > 12 || a < 1 || a > 31)
                            return null;
                        let i = new Date(r, n - 1, a);
                ...
                if (
                    !(await y.prisma.companyMember.findFirst({
                        where: {
                            companyId: a,
                            userId: o,
                            isActive: !0,
                    ...
                    return C('Cliente não encontrado nesta empresa.', 404);
                let m = await N(),
                    f = await y.prisma.user.update({
                        where: { id: o },
                        data: { name: l, email: u, phone: d, birthday: p },
                        select: { id: !0 },
                    });
                return (
                    m &&
                        (await y.prisma.companyMember.update({
                            where: {
                                companyId_userId: { companyId: a, userId: o },
                            },
                            data: { lastUnitId: m },
                            select: { id: !0 },
                        })),
                    (r = { id: f.id }),
                    w.NextResponse.json({ ok: !0, data: r }, void 0)
                );
            } catch (t) {
        ...
        e.s(['PATCH', () => T, 'dynamic', 0, 'force-dynamic'], 907112);
        var D = e.i(907112);
        let M = new t.AppRouteRouteModule({
                definition: {
                    kind: r.RouteKind.APP_ROUTE,
                    page: '/api/admin/clients/[id]/route',
                    pathname: '/api/admin/clients/[id]',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/admin/clients/[id]/route.ts',
        ...
        async function q(e, t, n) {
            ...
            let w = '/api/admin/clients/[id]/route';
            w = w.replace(/\/index$/, '') || '/';
            let R = await M.prepare(e, t, {
                srcPage: w,
            ...
            let {
                ...
                q = !!(I.dynamicRoutes[P] || I.routes[S]),
                O = async () => (
            ...
            try {
                let o = async (e) =>
                    ...
                    l = async (a) => {
                        ...
                        return (
                            (s && q) || c.delete(v.NEXT_CACHE_TAGS_HEADER),
                            !d.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__15cb2752._.js.map
