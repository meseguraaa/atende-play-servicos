module.exports = [
    ...
    (e) => {
        ...
        async function h(e, r, a) {
            ...
            let f = { ...m, ...e.header },
                ...
                y = !0;
            if (w.has('b64') && 'boolean' != typeof (y = m.b64))
                throw new i.JWSInvalid(
            ...
            let x = !1;
            ('function' == typeof r && ((r = await r(m, e)), (x = !0)),
            ...
            let R = await (0, p.normalizeKey)(r, v);
            if (!(await o(v, R, n, S)))
            ...
            if (y)
                ...
                h =
                    'string' == typeof e.payload
                        ...
                        : e.payload;
            let E = { payload: h };
            return (void 0 !== e.protected && (E.protectedHeader = m),
    ...
    (e) => {
        ...
        async function b(e) {
            let t = String(e.headers.get('authorization') ?? '').trim(),
            ...
            let o = 'string' == typeof a?.companyId ? E(a.companyId) : '',
                i =
                    (function (e, t) {
                        let r = t.toLowerCase();
        ...
        async function A(e, t) {
            try {
                ...
                let { itemId: a } = await t.params,
                    n = E(a);
                if (!n)
                    return g.NextResponse.json(
                        { ok: !1, error: 'itemId inválido' },
                        { status: 400, headers: R() }
                    );
                let o = r.companyId,
                    i = r.sub,
                    s = await S.prisma.orderItem.findFirst({
                        where: { id: n, companyId: o },
                        select: {
                            id: !0,
                            orderId: !0,
                            order: {
                                select: { id: !0, status: !0, clientId: !0 },
                            },
                        },
                    });
                if (!s?.id || !s?.orderId)
                    return g.NextResponse.json(
                        { ok: !1, error: 'Item não encontrado' },
                        { status: 404, headers: R() }
                    );
                let d = E(s.order?.clientId);
                if (!d || d !== i)
                    return g.NextResponse.json(
                        { ok: !1, error: 'Item não encontrado' },
                        { status: 404, headers: R() }
                    );
                let l = E(s.order?.status).toUpperCase();
                if ('PENDING_CHECKIN' !== l)
                    return g.NextResponse.json(
                        {
                            ok: !1,
                ...
                let c = await S.prisma.$transaction(async (e) => {
                    await e.orderItem.delete({ where: { id: s.id } });
                    let t = await e.orderItem.findMany({
                    ...
                    if (!t.length) {
                        ...
                        return { mode: 'canceled', order: t };
                    }
                    let r = t.reduce((e, t) => {
                            let r = Number(t.totalPrice ?? 0);
                            return e + (Number.isFinite(r) ? r : 0);
                        }, 0),
                        a = await e.order.update({
                            where: { id: s.orderId },
                            data: { totalAmount: r },
                            select: { id: !0, status: !0, totalAmount: !0 },
                        });
                    return { mode: 'updated', order: a };
                });
                return g.NextResponse.json(
                    { ok: !0, ...c },
        ...
        let N = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/mobile/cart/items/[itemId]/route',
                    pathname: '/api/mobile/cart/items/[itemId]',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/mobile/cart/items/[itemId]/route.ts',
        ...
        async function J(e, t, a) {
            ...
            let g = '/api/mobile/cart/items/[itemId]/route';
            g = g.replace(/\/index$/, '') || '/';
            ...
            let {
                ...
                _ = !!(A.dynamicRoutes[H] || A.routes[k]),
                j = async () => (
                    (null == C ? void 0 : C.render404)
                        ? await C.render404(e, t, b, !1)
            ...
            try {
                let i = async (e) =>
                    ...
                    d = async (n) => {
                        ...
                        return (
                            (s && _) || u.delete(w.NEXT_CACHE_TAGS_HEADER),
                            !c.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8c13d1d2._.js.map
