module.exports = [
    ...
    (e) => {
        ...
        async function h(e, r, a) {
            ...
            let m = { ...y, ...e.header },
                ...
                f = !0;
            if (w.has('b64') && 'boolean' != typeof (f = y.b64))
                throw new o.JWSInvalid(
        ...
        class u {
            ...
            async sign(e, r) {
                ...
                let y = { ...this.#t, ...this.#r },
                    ...
                    w = !0;
                if (m.has('b64') && 'boolean' != typeof (w = this.#t.b64))
                    throw new s.JWSInvalid(
                        'The "b64" (base64url-encode payload) Header Parameter must be a boolean'
                    );
                let { alg: f } = y;
    ...
    (e) => {
        ...
        async function E(e) {
            ...
            if (!d) return A('missing_company_id', 400);
            let l = String(s?.email ?? '')
                    .trim()
                    .toLowerCase(),
                c = R(s?.password);
            if (!l) return A('missing_email', 400);
            if (!c) return A('missing_password', 400);
            let p = await S.prisma.company.findFirst({
                where: { isActive: !0, OR: [{ id: d }, { slug: d }] },
                select: { id: !0 },
            });
            if (!p)
                return (await S.prisma.company.findFirst({
                    where: { OR: [{ id: d }, { slug: d }] },
                    select: { id: !0, isActive: !0 },
                }))
                    ? A('company_inactive', 403)
                    : A('company_not_found', 404);
            let u = p.id,
                h = await S.prisma.user.findFirst({
                    where: { email: { equals: l, mode: 'insensitive' } },
                    select: {
                        id: !0,
                        name: !0,
                        email: !0,
            ...
            if (!h) return A('invalid_credentials', 401);
            if (!h.isActive) return A('user_inactive', 403);
            let y = R(h.passwordHash);
            if (!y) return A('password_login_not_enabled', 403);
            if (!(await b.default.compare(c, y)))
                return A('invalid_credentials', 401);
            let m = await S.prisma.companyMember.findFirst({
                where: { companyId: u, userId: h.id, isActive: !0 },
                select: {
                    role: !0,
                    companyId: !0,
                    lastUnitId: !0,
            ...
            if (!m || !1 === m.isActive) return A('company_not_allowed', 403);
            let w =
                    'OWNER' === (r = m.role) || 'ADMIN' === r
                        ? 'ADMIN'
                        : 'STAFF' === r
                          ? 'BARBER'
                          : 'CLIENT',
        ...
        let C = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/mobile/auth/login/route',
                ...
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/mobile/auth/login/route.ts',
        ...
        async function N(e, t, a) {
            ...
            let v = '/api/mobile/auth/login/route';
            v = v.replace(/\/index$/, '') || '/';
            ...
            let {
                ...
                N = !!(I.dynamicRoutes[j] || I.routes[P]),
                O = async () => (
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b5aa166b._.js.map
