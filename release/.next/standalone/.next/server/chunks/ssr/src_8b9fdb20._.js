module.exports = [
    ...
    (a) => {
        ...
        function c(a, b) {
            return ('string' == typeof a?.error ? a.error : '') || b;
        }
        async function d(a) {
            let d = await fetch('/api/admin/professionals', {
                    method: 'POST',
            ...
            return d.ok && e?.ok
                ? { ok: !0, data: e.data }
                : { ok: !1, error: c(e, 'Erro ao criar profissional.') };
        }
        async function e(a, d) {
            let e = await fetch(
                    `/api/admin/professionals/${encodeURIComponent(a)}`,
                    {
                        method: 'PATCH',
                        headers: { 'Content-Type': 'application/json' },
                        cache: 'no-store',
            ...
            return e.ok && f?.ok
                ? { ok: !0, data: f.data }
                : { ok: !1, error: c(f, 'Erro ao atualizar profissional.') };
        }
        a.s(['createProfessional', () => d, 'updateProfessional', () => e]);
    },
    261519,
    (a) => {
        'use strict';
        ...
        async function y(a) {
            let b = new FormData();
            (b.append('file', a), b.append('module', 'PROFESSIONALS'));
        ...
        function z({ barber: a, units: f, selectedUnitIds: g }) {
            let i = (0, d.useRouter)(),
                z = (0, c.useRef)(null),
                [A, B] = (0, c.useState)(!1),
                [C, D] = (0, c.useState)(!1),
                [E, F] = (0, c.useState)(!1),
                [G, H] = (0, c.useState)(a.name ?? ''),
                [I, J] = (0, c.useState)(a.email ?? ''),
                [K, L] = (0, c.useState)(a.phone ?? ''),
                [M, N] = (0, c.useState)(a.imageUrl ?? ''),
                [O, P] = (0, c.useState)(''),
                Q = (f?.length ?? 0) > 0,
                R = (0, c.useMemo)(
                    () => f.find((a) => a.isActive)?.id ?? '',
                    [f]
                ),
                [S, T] = (0, c.useState)(() => (g?.[0] ?? '') || R);
            async function U(a) {
                ...
                let b = await y(a);
                (F(!1), b.ok)
                    ? (N(b.data.url), e.toast.success('Imagem enviada!'))
                    : e.toast.error(b.error);
            }
            (0, c.useEffect)(() => {
                !A ||
                    (H(a.name ?? ''),
            ...
            }, [A, a.name, a.email, a.phone, a.imageUrl, g, R]);
            let V = (0, c.useMemo)(() => {
                    let a = M.trim();
                    if (!a) return null;
                    let b = String(a ?? '').trim();
            ...
            async function Y(b) {
                ...
                (D(!1), d.ok)
                    ? (e.toast.success('Alterações salvas!'),
                      B(!1),
                      i.refresh())
                    : e.toast.error(d.error);
            }
            return (0, b.jsxs)(j.Dialog, {
                open: A,
                onOpenChange: (a) => {
                    W || B(a);
                },
        ...
        let A = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'],
            B = [
                'Domingo',
                'Segunda-feira',
                'Terça-feira',
        ...
        function D({ row: a, units: j }) {
            let k = (0, d.useRouter)(),
                ...
                q = a.selectedUnitIds ?? [],
                r = (0, c.useMemo)(() => {
                    if (a.unitsSummaryLabel) return a.unitsSummaryLabel;
                    let b = a.linkedUnits ?? [];
                    return 0 === b.length
                        ? 'Sem unidade'
                        : b.length <= 2
                          ? b.map((a) => a.name).join(' • ')
                          : `${b.length} unidades`;
                }, [a.unitsSummaryLabel, a.linkedUnits]),
                s = (0, c.useMemo)(
                    () =>
                        (a.linkedUnits ?? [])
                            .slice()
                ...
                t = a.weeklyAvailabilities ?? [],
                u = a.dailyAvailabilities ?? [],
                v = a.reviewStats ?? null,
                w = (0, c.useMemo)(
                    () =>
                        (function (a) {
                            let b = new Map();
                ...
                x = v ? v.avgRating.toFixed(2) : '—';
            async function y() {
                if (l) return;
                n(!0);
                let b = await (0, m.updateProfessional)(a.id, { isActive: !p });
                (n(!1), b.ok)
                    ? (e.toast.success(
                          p
            ...
            return (0, b.jsxs)(i.AccordionItem, {
                value: a.id,
                className:
    ...
    (a) => {
        ...
        function w({ units: a }) {
            ...
            async function T(a) {
                ...
                let b = await v(a);
                (D(!1), b.ok)
                    ? (R(b.data.url), d.toast.success('Imagem enviada!'))
                    : d.toast.error(b.error);
            }
            (0, c.useEffect)(() => {
                y && P((a) => a || N);
            }, [y, N]);
            ...
            async function V(b) {
                if ((b.preventDefault(), A || C)) return;
                if (!M)
                    return void d.toast.error(
                        'Crie pelo menos 1 unidade antes de cadastrar profissionais.'
                    );
                let c = a.find((a) => a.id === O) || null;
                if (!c || !c.isActive)
                    return void d.toast.error('Selecione uma unidade ativa.');
                B(!0);
                let e = await (0, j.createProfessional)({
                    name: E.trim(),
                    email: G.trim().toLowerCase(),
                    phone: I.trim(),
                    password: K,
                    unitIds: [O],
                    imageUrl: U,
                ...
                (B(!1), e.ok)
                    ? (d.toast.success('Profissional criado com sucesso!'),
                      z(!1),
                      S(),
                      w.refresh())
                    : d.toast.error(e.error);
            }
            let W = A || C,
                X = !M || !O;
