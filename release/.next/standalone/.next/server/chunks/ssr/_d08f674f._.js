module.exports = [
    ...
    (a) => {
        ...
        function j(a) {
            return 'HAS_ACTIVE_PLAN' === a ? 'Tem plano ativo' : a;
        }
        function k({ initialData: a, error: k, initialCreateMode: l }) {
            let m = (0, e.useRouter)(),
                n = (0, e.usePathname)(),
                o = (0, e.useSearchParams)(),
                p = a?.units ?? [],
                q = a?.activeUnitId ?? '',
                r = a?.levels ?? ['BRONZE', 'PRATA', 'OURO', 'DIAMANTE'],
                s = a?.ruleTypes ?? ['HAS_ACTIVE_PLAN'],
                t = a?.rules ?? [],
                u = t.length > 0,
                v = 1 === p.length,
                w = c.useMemo(() => p.find((a) => a.id === q) ?? null, [p, q]),
                [x, y] = c.useState(q),
                [z, A] = c.useState(!1),
                B = o?.get('create');
            function C(a) {
                let b = new URLSearchParams(o?.toString() ?? '');
            ...
            async function F(a) {
                try {
                    A(!0);
                    let b = await fetch('/api/admin/client-levels/rules', {
                            method: 'POST',
                            body: a,
                        }),
                        c = await b.json();
                    if (!b.ok || !c.ok) {
                        let a = c.ok ? 'Falha ao salvar.' : c.error;
                        return (f.toast.error(a), !1);
                    }
                    return !0;
                } catch (a) {
                    return (
            ...
            async function G(a) {
                ...
                (b.set('intent', 'create'),
                    b.set('unitId', q),
                    (await F(b)) &&
                        (f.toast.success('Regra criada.'), E(), m.refresh()));
            }
            async function H(a, b) {
                if ((b.preventDefault(), !q)) return;
                let c = new FormData(b.currentTarget);
                (c.set('intent', 'update'),
                    c.set('unitId', q),
                    c.set('ruleId', a),
                    (await F(c)) &&
                        (f.toast.success('Regra atualizada.'), m.refresh()));
            }
            async function I(a) {
                if (!q) return;
                let b = new FormData();
                (b.set('intent', 'delete'),
                    ...
                    b.set('ruleId', a),
                    (await F(b)) &&
                        (f.toast.success('Regra excluída.'), m.refresh()));
            }
            return (
                c.useEffect(() => {
                    y(q);
...
//# sourceMappingURL=_d08f674f._.js.map
