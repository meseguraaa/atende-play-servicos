module.exports = [
    ...
    (e) => {
        ...
        async function b(e) {
            ...
            if (
                !(await w.prisma.companyMember.findFirst({
        ...
        async function P(e) {
            let t = new Date(),
                r = await w.prisma.order.findFirst({
                    where: {
                        id: e.orderId,
            ...
            if (!r?.id) return null;
            let i = String(r.status ?? '')
                    .toUpperCase()
                    .trim(),
                n = r.reservedUntil instanceof Date ? r.reservedUntil : null;
            return 'PENDING_CHECKIN' === i && n && n.getTime() <= t.getTime()
                ? (await w.prisma.$transaction(async (e) => {
                      if (!r.inventoryRevertedAt) {
                          for (let t of (r.items ?? []).filter(
                              (e) => e.productId && Number(e.quantity ?? 0) > 0
                          )) {
                              let r = String(t.productId),
                                  i = Math.max(1, Number(t.quantity ?? 1));
                              await e.product.update({
                                  where: { id: r },
                                  data: { stockQuantity: { increment: i } },
                              });
                          }
                          await e.order.update({
                              where: { id: r.id },
                              data: { inventoryRevertedAt: t },
                              select: { id: !0 },
                          });
                      }
                      await e.order.update({
        ...
        async function B(e) {
            ...
            let t = await w.prisma.customerLevelState.findFirst({
                where: {
                    companyId: e.companyId,
                    userId: e.userId,
                    unitId: e.unitId,
        ...
        async function M(e) {
            let t = e.timeZone ?? 'America/Sao_Paulo',
                ...
                n = e.clientId
                    ? w.prisma.user.findUnique({
                          where: { id: e.clientId },
                          select: { id: !0, birthday: !0 },
                      })
                    : Promise.resolve(null),
                [a, o] = await Promise.all([
                    w.prisma.product.findFirst({
                        where: {
                            id: e.productId,
                            isActive: !0,
                            companyId: e.companyId,
            ...
            if (!a) throw Error('Produto não encontrado.');
            let s = await w.prisma.productDiscountByLevel.findMany({
                    where: { productId: a.id, companyId: e.companyId },
                    select: { level: !0, discountPct: !0 },
                }),
                d = new Map();
            for (let e of s) {
                let t = O(e.level);
                t && d.set(t, _(Number(e.discountPct)));
            }
            function l(e) {
                for (let t of C[e])
                    if (d.has(t)) return { level: t, discountPct: d.get(t) };
                return { level: 'BRONZE', discountPct: 0 };
            }
            let u = Number(a.price),
                c = !1;
            if (o?.birthday && a.birthdayBenefitEnabled) {
                var p;
                let e,
                    i = T(r, t),
                    n = T(o.birthday, t),
                    a = x(i.year, n.month, n.day),
                    s = S(a, -3),
                    d = S(a, 3);
                ((p = x(i.year, i.month, i.day)),
                    (c = (e = p.getTime()) >= s.getTime() && e <= d.getTime()));
            }
            if (c && a.birthdayBenefitEnabled) {
                let e = l(O(a.birthdayPriceLevel) ?? 'DIAMANTE'),
                    t = U(u, e.discountPct);
                return {
                    unitId: a.unitId,
                    basePrice: u,
            ...
            return {
                ...
                appliedLevel: m.level,
                appliedBecause: m.discountPct > 0 ? 'LEVEL' : 'BASE',
                inBirthdayWindow: !1,
            };
        }
        async function L() {
            ...
            try {
                ...
                let a = (function (e) {
                        let t = String(process.env.APP_PUBLIC_BASE_URL ?? '')
                            ...
                            .replace(/\/+$/, '');
                        if (t) return t;
                        let r = A(e, 'x-forwarded-host') || A(e, 'host') || '',
                            i =
                                A(e, 'x-forwarded-proto') ||
                ...
                await P({ companyId: n, clientId: i.sub, orderId: s });
                let d = await w.prisma.order.findFirst({
                    where: { id: s, companyId: n, clientId: i.sub },
                    select: {
                        id: !0,
                        status: !0,
                        createdAt: !0,
                ...
                let l = i.sub,
                    ...
                    c = await Promise.all(
                        (d.items ?? []).map(async (e) => {
                            let t = Math.max(1, Number(e.quantity ?? 1)),
                                r = D(Number(e.unitPrice)),
                                i = D(Number(e.totalPrice));
                            if (!e.productId)
                                return {
                                    id: e.id,
                                    productId: null,
                            ...
                            try {
                                ((s = await M({
                                    ...
                                    s?.unitId &&
                                        d.unitId &&
                                        String(s.unitId) !== String(d.unitId) &&
                                        (s = null));
                            } catch {
                                s = null;
                            }
                                ...
                                return {
                                    ...
                                    product: e.product
                                        ? {
                                              ...
                                              category:
                                                  e.product.category ?? null,
                                          }
                                        : null,
                                };
                            let c = D(Number(s.basePrice)),
                                p = D(Number(s.finalPrice)),
                                m = _(Number(s.discountPct ?? 0)),
                                f = Number.isFinite(p) ? p : r,
                                h = D(f * t),
                                y =
                                    Number.isFinite(c) &&
                                    Number.isFinite(f) &&
                            ...
                            return {
                                ...
                                product: e.product
                                    ? {
                                          id: e.product.id,
                                          name: e.product.name,
                                          imageUrl: N(a, e.product.imageUrl),
                    ...
                    p = D(c.reduce((e, t) => e + D(t?.totalPrice), 0)),
                    m = {
                        id: d.id,
                        status: d.status,
                ...
                return (
                    console.error('[mobile orders/:id] error:', t),
                    g.NextResponse.json(
                        { error: 'server_error' },
        ...
        e.s(['GET', () => k, 'OPTIONS', () => L], 832814);
        var q = e.i(832814);
        let F = new t.AppRouteRouteModule({
                definition: {
                    kind: r.RouteKind.APP_ROUTE,
                    page: '/api/mobile/orders/[id]/route',
                    pathname: '/api/mobile/orders/[id]',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/mobile/orders/[id]/route.ts',
        ...
        async function K(e, t, i) {
            ...
            let g = '/api/mobile/orders/[id]/route';
            g = g.replace(/\/index$/, '') || '/';
            let w = await F.prepare(e, t, {
                srcPage: g,
            ...
            let {
                ...
                D = !!(P.dynamicRoutes[U] || P.routes[S]),
                B = async () => (
            ...
            if (D && !b) {
                let e = !!P.routes[S],
                    t = P.dynamicRoutes[U];
                if (t && !1 === t.fallback && !e) {
                    if (A.experimental.adapterPath) return await B();
                    throw new R.NoFallbackError();
                }
            }
            let M = null;
            !D || F.isDev || b || (M = '/index' === (M = S) ? '/' : M);
            let L = !0 === F.isDev || !D,
                k = D && !L;
            _ &&
                O &&
                ...
                j = {
                    ...
                    renderOpts: {
                        ...
                        onInstrumentationRequestError: (t, r, i) =>
                            F.onRequestError(e, t, i, C),
                    },
                    sharedContext: { buildId: I },
                },
                Z = new l.NodeNextRequest(e),
                K = new l.NodeNextResponse(t),
                W = u.NextRequestAdapter.fromNodeNextRequest(
                    Z,
                    (0, u.signalFromNodeResponse)(t)
                );
            try {
                let o = async (e) =>
                        F.handle(W, j).finally(() => {
                            if (!e) return;
                            e.setAttributes({
                    ...
                    d = async (n) => {
                        ...
                        let l = async ({ previousCacheEntry: r }) => {
                                try {
                                    ...
                                    let d = j.renderOpts.pendingWaitUntil;
                                    d &&
                                        i.waitUntil &&
...
//# sourceMappingURL=c2dd8_next_dist_esm_build_templates_app-route_a1c52e8e.js.map
