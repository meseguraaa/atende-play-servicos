...
type AdminReportsOccupancyPageProps = {
    searchParams: Promise<{
        ...
        compareProfessionalId?: string;

        // compat (aceita params antigos sem quebrar links)
        barberId?: string;
...
export default async function AdminReportsOccupancyPage({
    ...
    // ✅ padrão do projeto: permissão por módulo
    ...
    const monthParam = getSingleParam(sp.month);

    // ✅ param novo (aceita legacy de barber)
    const professionalIdParam =
        getSingleParam(sp.professionalId) ?? getSingleParam(sp.barberId);
    const compareProfessionalIdParam =
        getSingleParam(sp.compareProfessionalId) ??
        getSingleParam(sp.compareBarberId);
    ...
    const professionalIdSafe =
        professionalIdParam &&
        professionals.some((p) => p.id === professionalIdParam)
            ? professionalIdParam
            : null;

    const compareProfessionalIdSafe =
        compareProfessionalIdParam &&
        compareProfessionalIdParam !== professionalIdSafe &&
        professionals.some((p) => p.id === compareProfessionalIdParam)
            ? compareProfessionalIdParam
            : null;
        ...
        compareProfessionalIdSafe
            ? prisma.appointment.findMany({
                  where: {
                      ...
                      status: { in: OCCUPANCY_STATUSES },
                      ...whereAppointmentUnit(activeUnitId),
                      professionalId: compareProfessionalIdSafe,
                  },
                  select: { scheduleAt: true },
              })
    ...
    const isSplit = !!compareProfessionalIdSafe;
    ...
    const selectedProfessionalName = professionalIdSafe
        ? professionals.find((p) => p.id === professionalIdSafe)?.name
        : null;

    const compareProfessionalName = compareProfessionalIdSafe
        ? professionals.find((p) => p.id === compareProfessionalIdSafe)?.name
        : null;
                ...
                >
                    <div className="grid gap-3 md:grid-cols-[1fr_1fr_1fr_auto] md:items-end">
                        ...
                        <div className="w-full [&_select]:h-12 [&_select]:min-h-12 [&_select]:py-2">
                            <ProfessionalFilter
                                professionals={professionals}
                                value={compareProfessionalIdSafe}
                                paramKey="compareProfessionalId"
                                label="Comparar com"
                            />
                        </div>
                ...
                <div className="rounded-xl border border-border-primary bg-background-tertiary px-4 py-3">
                    ...
                    {isSplit ? (
                        <p className="mt-1 text-[11px] text-content-tertiary">
                            {selectedProfessionalName ?? 'Profissional A'}:{' '}
                            {heatA.totalCount} •{' '}
                            {compareProfessionalName ?? 'Profissional B'}:{' '}
                            {heatB.totalCount}
                        </p>
                    ) : null}
                </div>
            ...
            <section className="rounded-xl border border-border-primary bg-background-tertiary p-4">
                {isSplit ? (
                    <div className="grid gap-6 lg:grid-cols-2">
                        <HeatmapTable
                            title={selectedProfessionalName ?? 'Profissional A'}
                        ...
                        <HeatmapTable
                            title={compareProfessionalName ?? 'Profissional B'}
