(globalThis.TURBOPACK || (globalThis.TURBOPACK = [])).push([
    ...
    (e) => {
        ...
        var b = e.i(14435),
            ...
            S = e.i(512710),
            k = e.i(136764);
        let _ = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'],
            E = [
                'Domingo',
                'Segunda',
                'Terça',
            ...
            D = (() => {
                ...
                for (let r = 0; r <= 23; r++)
                    for (let t = 0; t < 60; t += 30)
                        e.push(
                            `${String(r).padStart(2, '0')}:${String(t).padStart(2, '0')}`
                        );
                return e;
            })();
        function I(e) {
        ...
        function $(e) {
            return (
                ...
                }[e] ?? 'Algo deu errado. Tente novamente.'
            );
        }
        function P(e) {
            return /^([01]\d|2[0-3]):([0-5]\d)$/.test(e.trim());
        }
        function z(e) {
            let [r, t] = e.split(':').map(Number);
            return 60 * r + t;
        }
        function O() {
            let e = new Date(),
        ...
        function q(e) {
            ...
            let [r, t, a] = e.split('-').map(Number);
            if (!r || !t || !a) return null;
            let s = new Date(r, t - 1, a).getDay();
            return Number.isFinite(s) ? s : null;
        }
        function B(e) {
            return [...e].sort(
                (e, r) =>
                    z(e.startTime) - z(r.startTime) ||
                    z(e.endTime) - z(r.endTime)
            );
        }
        function V({
            unitId: e,
            ...
            let c,
                ...
                m = t.useMemo(() => {
                    let e = a || {};
                    return Array.from({ length: 7 }).map((r, t) => {
                        let a = e[t] || {
                            isActive: !1,
                            startTime: '',
                            endTime: '',
                ...
                u = t.useMemo(
                    () =>
                        m.some((e) => {
                            ...
                            let r = String(e.startTime || '').trim(),
                                t = String(e.endTime || '').trim();
                            return !!P(r) && !!P(t) && z(r) >= z(t);
                        }),
                    [m]
                ),
                [p, x] = t.useState([]),
                ...
                F = t.useCallback(async () => {
                    (h(!0), V(null), R.current && R.current.abort());
                    let r = new AbortController();
                    R.current = r;
                    try {
                        let t = await fetch(
                                `/api/admin/settings/units/${e}/exceptions`,
                                {
                                    method: 'GET',
                                    headers: {
                        ...
                        if (!t.ok || !a || !a.ok) {
                            let e = a ? I(a) : 'internal_error',
                                r = $(e);
                            (x([]), V(r));
                            return;
                        }
                        let s = Array.isArray(a.data) ? a.data : [];
                        x(
                            [...s].sort((e, r) =>
                                e.date === r.date ? 0 : e.date < r.date ? 1 : -1
                            )
                        );
                    } catch (e) {
            ...
            let [L, U] = t.useState(null);
            async function M(r) {
                if (r) {
                    U(r);
                    try {
                        let t = await fetch(
                                `/api/admin/settings/units/${e}/exceptions/${r}`,
                                {
                                    method: 'DELETE',
                                    headers: {
                                        'Content-Type': 'application/json',
                        ...
                        (v.toast.success('Exceção removida ✅'), await F());
                    } catch {
                        v.toast.error(
                ...
                Z = t.useMemo(
                    () =>
                        (function (e) {
                            ...
                            let [r, t, a] = e.split('-').map(Number);
                            if (!r || !t || !a) return null;
                            let s = new Date(r, t - 1, a);
                            return (0, w.isValid)(s) ? s : null;
                        })(W.date) ?? new Date(),
                    [W.date]
                );
            function ee() {
            ...
            function er(e, r) {
                X((t) => {
                    let a = [...t.intervals];
                    return ((a[e] = { ...a[e], ...r }), { ...t, intervals: a });
                });
            }
            function et(e) {
                let r = q(e);
                ...
                let s = String(t.startTime || '').trim(),
                    n = String(t.endTime || '').trim();
                if (!P(s) || !P(n)) return null;
                let i = z(s);
                return z(n) <= i ? null : { startTime: s, endTime: n };
            }
            async function ea(r) {
                r.preventDefault();
                let t = String(W.date || '').trim(),
                    a = W.mode;
                if (!t || !/^\d{4}-\d{2}-\d{2}$/.test(t))
                    return void v.toast.error('Informe uma data válida.');
                let s = [];
                if ('FULL_DAY' === a) {
                    let e = et(t);
                    if (!e) {
                        let e = q(t),
                            r = null !== e ? (E[e] ?? `Dia ${e}`) : 'o dia';
                        v.toast.error(
                            `N\xe3o d\xe1 pra bloquear o dia inteiro: ${r} n\xe3o est\xe1 com hor\xe1rio v\xe1lido no padr\xe3o semanal.`
                        );
                        return;
                    }
                    s = [e];
                } else {
                    let e = W.intervals.map((e) => ({
                    ...
                    for (let r = 0; r < e.length; r++) {
                        ...
                        if (!P(t.startTime) || !P(t.endTime))
                            return void v.toast.error(
                                `Intervalo #${r + 1}: hor\xe1rios inv\xe1lidos.`
                            );
                        let a = z(t.startTime);
                        if (z(t.endTime) <= a)
                            return void v.toast.error(
                                `Intervalo #${r + 1}: o hor\xe1rio final deve ser maior que o inicial.`
                            );
                    }
                    if (
                        (function (e) {
                            ...
                            for (let e = 1; e < r.length; e++) {
                                let t = z(r[e - 1].endTime);
                                if (z(r[e].startTime) < t) return !0;
                            }
                            return !1;
                        })(e)
                    )
            ...
            return (0, r.jsxs)('div', {
                ...
                children: [
                    (0, r.jsx)(o.Dialog, {
                        open: H,
                        onOpenChange: (e) => (e ? G(!0) : ee()),
                        children: (0, r.jsxs)(o.DialogContent, {
                            variant: 'appointment',
                            overlayVariant: 'blurred',
                            showCloseButton: !0,
                            className: 'sm:max-w-[720px]',
                            children: [
                                (0, r.jsxs)(o.DialogHeader, {
                                    children: [
                                        (0, r.jsx)(o.DialogTitle, {
                                            size: 'modal',
                                            children: 'Criar exceção',
                                        }),
                                        (0, r.jsx)(o.DialogDescription, {
                                            size: 'modal',
                                            children:
                    ...
                    (0, r.jsxs)('form', {
                        ...
                        children: [
                            ...
                            (0, r.jsx)('div', {
                                ...
                                children: m.map((t) => {
                                    let a = P(String(t.startTime || '')),
                                        s = P(String(t.endTime || '')),
                                        i =
                                            t.isActive &&
                                            a &&
                                            s &&
                                            z(t.startTime) >= z(t.endTime);
                                    return (0, r.jsxs)(
                                        'div',
                                        {
                                            className: (0, g.cn)(
        ...
        function J(e) {
            let r = new Date(e.createdAt),
                t =
                    e.address ||
                    [e.street, e.number ? `, ${e.number}` : '']
                        .join('')
                        .trim() ||
                    '—';
            return {
                id: e.id,
                name: e.name,
                phone: e.phone?.trim() ? e.phone : '—',
                address: t || '—',
                isActive: !!e.isActive,
                createdAt: Number.isNaN(r.getTime()) ? new Date() : r,
            };
        }
        function Q(e) {
            let r = new Date(e.createdAt);
            return {
                ...
                isActive: !!e.isActive,
                permissions: {
                    canAccessDashboard: !!e.permissions?.canAccessDashboard,
                    canAccessReports: !!e.permissions?.canAccessReports,
                    canAccessCheckout: !!e.permissions?.canAccessCheckout,
                    canAccessAppointments:
                        !!e.permissions?.canAccessAppointments,
                    canAccessProfessionals:
                        !!e.permissions?.canAccessProfessionals,
                    canAccessServices: !!e.permissions?.canAccessServices,
                    canAccessReviews: !!e.permissions?.canAccessReviews,
                    canAccessProducts: !!e.permissions?.canAccessProducts,
                    canAccessPartners: !!e.permissions?.canAccessPartners,
                    canAccessClients: !!e.permissions?.canAccessClients,
                    canAccessClientLevels:
                        !!e.permissions?.canAccessClientLevels,
                    canAccessFinance: !!e.permissions?.canAccessFinance,
                    canAccessSettings: !!e.permissions?.canAccessSettings,
                },
            };
        }
        function Y(e) {
        ...
        function W(e) {
            ...
            if (r.length <= 10) {
                let e = a.slice(0, 4),
                    r = a.slice(4, 8);
                return a.length <= 4 ? `(${t}) ${e}` : `(${t}) ${e}-${r}`;
            }
            let s = a.slice(0, 5),
                n = a.slice(5, 9);
            return a.length <= 5 ? `(${t}) ${s}` : `(${t}) ${s}-${n}`;
        }
        function X(e) {
        ...
        function ee(e) {
            return {
                canAccessDashboard: !!e.canAccessDashboard,
                canAccessReports: !!e.canAccessReports,
                canAccessCheckout: !!e.canAccessCheckout,
                canAccessAppointments: !!e.canAccessAppointments,
                ...
                canAccessClients: !!e.canAccessClients,
        ...
        function er(e) {
            let { label: t, value: a, disabled: s, onToggle: n } = e,
                i = a
                    ? 'border-emerald-500/40 bg-emerald-500/10'
                    : 'border-destructive/40 bg-destructive/10',
                o = s ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer';
            return (0, r.jsxs)('button', {
                type: 'button',
                onClick: s ? void 0 : n,
                disabled: s,
                className: `flex items-center justify-between gap-3 rounded-xl border px-3 py-2 transition select-none ${i} ${o}`,
                'aria-pressed': a,
                children: [
                    (0, r.jsx)('span', {
                    ...
                    (0, r.jsx)('span', {
                        className: `text-[11px] font-medium ${a ? 'text-emerald-500' : 'text-content-secondary'}`,
                        children: a ? 'Liberado' : 'Bloqueado',
                    }),
                ],
            });
        }
                ...
                children: [
                    (0, r.jsx)('div', {
                        ...
                        children: (0, r.jsx)(t, {
                            width: 20,
                            height: 20,
                            className: `${o ? 'text-content-secondary/50' : 'text-content-brand'} ${a ?? ''}`,
                        }),
                    }),
                    (0, r.jsx)(i.Input, {
                        ...l,
        ...
        function ea() {
            ...
            t.useEffect(() => {
                ...
                return (
                    (async function () {
                        (y(!0), w(null));
                        try {
                            let t = await fetch('/api/admin/settings/company', {
                                    method: 'GET',
                                    headers: {
                                        'Content-Type': 'application/json',
                                    },
            ...
            t.useEffect(() => {
                ...
                return (
                    (async function () {
                        k(!0);
                        try {
                            let t = await fetch('/api/admin/settings/units', {
                                    method: 'GET',
                                    headers: {
                                        'Content-Type': 'application/json',
                                    },
                            ...
                            (C(n),
                                P((e) => {
                                    let r = { ...e };
                                    for (let e of n) r[e.id] || (r[e.id] = G());
                                    return r;
                                }));
                        } catch (r) {
                            if (!e || r?.name === 'AbortError') return;
            ...
            let [q, B] = t.useState([]),
                ...
                [eo, ec] = t.useState({}),
                [el, ed] = t.useState({}),
                em = t.useCallback(async () => {
                    es(!0);
                    try {
                        let e = await fetch('/api/admin/settings/admins', {
                                method: 'GET',
                                headers: { 'Content-Type': 'application/json' },
                            }),
                            r = null;
                        ...
                        (t.sort((e, r) =>
                            e.isOwner !== r.isOwner
                                ? e.isOwner
                                    ? -1
                                    : 1
                                : r.createdAt.getTime() - e.createdAt.getTime()
                        ),
                            B(t),
                            ei((e) => {
                                let r = { ...e };
                                for (let e of t)
                                    e.isOwner || (r[e.id] = ee(e.permissions));
                                return r;
                            }),
                            ec((e) => {
                                let r = { ...e };
                                for (let e of t) e.isOwner || (r[e.id] = !1);
                                return r;
                            }));
                    } catch {
                        (v.toast.error(
            ...
            async function eu(e) {
                let r = q.find((r) => r.id === e) ?? null;
                if (!r || r.isOwner) return;
                let t = en[e];
                if (t && eo[e]) {
                    ed((r) => ({ ...r, [e]: !0 }));
                    try {
                        let r = await fetch(`/api/admin/settings/admins/${e}`, {
                                method: 'PATCH',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ permissions: t }),
                            }),
                            a = null;
                        try {
                            a = await r.json();
                        ...
                        (v.toast.success('Permissões salvas.'),
                            B((r) =>
                                r.map((r) =>
                                    r.id === e
                                        ? { ...r, permissions: ee(t) }
            ...
            t.useEffect(() => {
                ...
                return (
                    (async () => {
                        try {
                            es(!0);
                            let t = await fetch('/api/admin/settings/admins', {
                                    method: 'GET',
                                    headers: {
                                        'Content-Type': 'application/json',
                                    },
            ...
            let [ep, ex] = t.useState({
                ...
                [eb, eh] = t.useState('idle'),
                [ef, ev] = t.useState(null),
                eg = t.useRef(''),
                ey = t.useRef(null);
            t.useEffect(() => {
                let e = Y(ep.cep).slice(0, 8);
                if (e.length < 8) {
                    ...
                    (async () => {
                        try {
                            ...
                            let t = await fetch(
                                ...
                                a = await t.json();
                            if (!t.ok) throw Error('FETCH_FAILED');
                            if ('erro' in a && a.erro) {
                                ((eg.current = ''),
                                    eh('error'),
                                    ev('CEP não encontrado.'),
                                    ex((e) => ({
                                        ...e,
                                        street: '',
                                        neighborhood: '',
            ...
            let [ej, eN] = t.useState(!1),
                [eA, ew] = t.useState(null),
                [eT, eC] = t.useState(!1),
                [eS, ek] = t.useState({
                    name: '',
                    phone: '',
                ...
                [e_, eE] = t.useState('idle'),
                [eD, eI] = t.useState(null),
                e$ = t.useRef(''),
                eP = t.useRef(null);
            async function ez(e) {
                if (D[e]) return D[e];
                try {
                    let r = await fetch(`/api/admin/settings/units/${e}`, {
                            method: 'GET',
                            headers: { 'Content-Type': 'application/json' },
                        }),
                        t = null;
                    ...
                    if (!r.ok || !t || !t.ok) {
                        let e = t ? H(t) : 'internal_error';
                        return (v.toast.error(U(e)), null);
                    }
                    return (I((r) => ({ ...r, [e]: t.data })), t.data);
                } catch {
                    return (
                        v.toast.error(
            ...
            async function eO(e) {
                ...
                let t = Y(r.cep || '').slice(0, 8);
                ((e$.current = t || ''),
                    eE(8 === t.length ? 'success' : 'idle'),
                    eI(null),
                    ew(e),
                    ek({
                        name: r.name || '',
            ...
            function eq() {
                (eN(!1),
                    ew(null),
                    eC(!1),
                    eP.current && eP.current.abort(),
                    eE('idle'),
                    eI(null),
                    (e$.current = ''));
            }
            async function eB(e) {
                if ((e.preventDefault(), !eA)) return;
                let r = eS.name.trim();
                if (!r) return void v.toast.error('Informe o nome da unidade.');
                let t = Y(eS.phone);
                if (eS.phone.trim() && t.length < 10)
                    return void v.toast.error('Informe um telefone válido.');
                let a = Y(eS.cep).slice(0, 8);
                if (eS.cep.trim() && 8 !== a.length)
                    return void v.toast.error('Informe um CEP válido.');
                eC(!0);
                try {
                    let e = await fetch(`/api/admin/settings/units/${eA}`, {
                            method: 'PUT',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                                name: r,
                                phone: eS.phone.trim() || null,
                                cep: a || null,
                                street: eS.street.trim() || null,
                                number: eS.number.trim() || null,
                                complement: eS.complement.trim() || null,
                                neighborhood: eS.neighborhood.trim() || null,
                                city: eS.city.trim() || null,
                                state: eS.state.trim() || null,
                    ...
                    let s = t.data;
                    (I((e) => ({ ...e, [s.id]: s })),
                        C((e) => e.map((e) => (e.id === s.id ? J(s) : e))),
                        v.toast.success('Unidade atualizada.'),
                        eq());
                } catch {
                    v.toast.error(
            ...
            let [eV, eR] = t.useState({
                ...
                [eU, eM] = t.useState(!1);
            async function eH(r) {
                (r.preventDefault(), w(null));
                let t = e.name.trim();
                if (!t) return void w('Informe o nome da empresa.');
                N(!0);
                try {
                    let r = await fetch('/api/admin/settings/company', {
                            method: 'PUT',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                                name: t,
                    ...
                    (d({
                        ...
                        v.toast.success('Empresa salva.'));
                } catch {
                    let e = 'Não foi possível salvar. Verifique sua conexão.';
                    (w(e), v.toast.error(e));
                } finally {
                    N(!1);
                }
            ...
            async function eG(e) {
                ...
                if (!r) return void v.toast.error('Informe o nome da unidade.');
                let t = Y(ep.phone);
                if (ep.phone.trim() && t.length < 10)
                    return void v.toast.error('Informe um telefone válido.');
                let a = Y(ep.cep).slice(0, 8);
                if (ep.cep.trim() && 8 !== a.length)
                    return void v.toast.error('Informe um CEP válido.');
                E(!0);
                try {
                    var s;
                    let e,
                        ...
                        m,
                        u,
                        p = await fetch('/api/admin/settings/units', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                                name: r,
                                phone: ep.phone.trim() || null,
                                cep: a || null,
                                street: ep.street.trim() || null,
                                number: ep.number.trim() || null,
                                complement: ep.complement.trim() || null,
                                neighborhood: ep.neighborhood.trim() || null,
                                city: ep.city.trim() || null,
                                state: ep.state.trim() || null,
                                address:
                                    ((s = {
                                        street: ep.street,
                                        number: ep.number,
                                        complement: ep.complement,
                    ...
                    if (!p.ok || !x || !x.ok) {
                        let e = x ? H(x) : 'internal_error',
                            r = U(e);
                        v.toast.error(r);
                        return;
                    }
                    I((e) => ({ ...e, [x.data.id]: x.data }));
                    let b = J(x.data);
                    (C((e) => [b, ...e]),
                        P((e) => ({ ...e, [b.id]: e[b.id] || G() })),
                        ex({
                        ...
                        ev(null),
                        (eg.current = ''),
                        v.toast.success('Unidade criada.'));
                } catch {
                    v.toast.error(
            ...
            async function eJ(e) {
                ...
                try {
                    ...
                    if (!e.ok || !n || !n.ok) {
                        let e = n ? H(n) : 'internal_error';
                        v.toast.error(M(e));
                        return;
                    }
                    (v.toast.success('Administrador criado.'),
                        eR({ name: '', email: '', phone: '', password: '' }),
                        eL({
                            canAccessDashboard: !0,
                            canAccessReports: !1,
                            canAccessCheckout: !1,
            ...
            async function eQ(e, r) {
                ...
                for (let e = 0; e < 7; e++) {
                    ...
                    let a = String(r.startTime || '').trim(),
                        s = String(r.endTime || '').trim();
                    if (!a || !s)
                        return void v.toast.error(
                            `Informe hor\xe1rio completo em ${R[e] ?? `Dia ${e}`}.`
                        );
                    if (a >= s)
                        return void v.toast.error(
                            `Hor\xe1rio inv\xe1lido em ${R[e] ?? `Dia ${e}`}: "Das" deve ser menor que "At\xe9".`
                        );
                }
                O((r) => ({ ...r, [e]: !0 }));
                try {
                    let r = {
                            weekly: Array.from({ length: 7 }).map((e, r) => {
                                let a = t[r] ?? {
                                    isActive: !1,
                                    startTime: '',
                                    endTime: '',
                                ...
                                return {
                                    ...
                                    startTime:
                                        ...
                                        null,
                                    endTime:
                                        String(a.endTime || '').trim() || null,
                                };
                            }),
                        },
                        a = await fetch(
                            `/api/admin/settings/units/${e}/weekly`,
                            {
                                method: 'PUT',
                                headers: { 'Content-Type': 'application/json' },
                    ...
                    if (!a.ok || !s || !s.ok) {
                        let e = s ? H(s) : 'internal_error';
                        v.toast.error(U(e));
                        return;
                    }
                    v.toast.success('Padrão semanal salvo.');
                } catch {
                    v.toast.error('Não foi possível salvar o padrão semanal.');
                } finally {
                    O((r) => ({ ...r, [e]: !1 }));
                }
            }
            async function eY(e, r) {
                let t = q.find((r) => r.id === e) ?? null;
                if (t) {
                    if (t.isOwner)
                        return void v.toast.error(
                    ...
                    B((t) =>
                        t.map((t) => (t.id === e ? { ...t, isActive: r } : t))
                    );
                    try {
                        let a = await fetch(`/api/admin/settings/admins/${e}`, {
                                method: 'PATCH',
                                headers: { 'Content-Type': 'application/json' },
                        ...
                        (v.toast.success(
                            r ? 'Admin ativado.' : 'Admin desativado.'
                        ),
                            await em());
                    } catch {
                        (v.toast.error(
