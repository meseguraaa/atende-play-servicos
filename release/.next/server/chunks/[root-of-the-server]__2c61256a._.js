    ...
    (e) => {
        ...
        async function h(e, r, a) {
            ...
            let w = { ...y, ...e.header },
                ...
                f = !0;
            if (m.has('b64') && 'boolean' != typeof (f = y.b64))
                throw new i.JWSInvalid(
            ...
            let x = !1;
            ('function' == typeof r && ((r = await r(y, e)), (x = !0)),
            ...
            if (f)
                ...
                h =
                    'string' == typeof e.payload
                        ...
                        : e.payload;
            let b = { payload: h };
            return (void 0 !== e.protected && (b.protectedHeader = y),
    ...
    (e) => {
        ...
        async function E(e) {
            ...
            let o =
                ...
                s = o || i;
            if (!s) throw Error('companyId_missing');
            if (
            ...
            return { sub: n, role: a?.role, companyId: s };
        }
        async function A() {
            return new g.NextResponse(null, { status: 204, headers: b() });
        }
        let I = S.z.object({
            appointmentId: S.z.string().min(1, 'appointmentId é obrigatório'),
        });
        async function C(e) {
            let t = b();
            try {
                ...
                if (!n.success)
                    return g.NextResponse.json(
                        { ok: !1, error: 'Dados inválidos.' },
                        { status: 400, headers: t }
                    );
                let { appointmentId: o } = n.data,
                    i = await R.prisma.appointment.findFirst({
                        where: { id: o, companyId: r.companyId },
                        select: {
                            id: !0,
                            clientId: !0,
                            reviewModalShown: !0,
                ...
                if (!i)
                    return g.NextResponse.json(
                        { ok: !1, error: 'Atendimento não encontrado.' },
                ...
                if (i.review || i.reviewModalShown) {
                    let e = g.NextResponse.json(
                        { ok: !0 },
                        { status: 200, headers: t }
                    );
                    return (e.headers.set('x-company-id', r.companyId), e);
                }
                await R.prisma.appointment.updateMany({
                    where: { id: o, companyId: r.companyId, clientId: r.sub },
                    data: { reviewModalShown: !0 },
                });
                let s = g.NextResponse.json(
                    { ok: !0 },
                    { status: 200, headers: t }
                );
                return (s.headers.set('x-company-id', r.companyId), s);
            } catch (n) {
                let e = String(n?.message ?? 'Erro ao atualizar.'),
                    r = e.toLowerCase(),
                    a =
                        r.includes('missing_token') ||
                        r.includes('invalid_token') ||
                        r.includes('jwt') ||
                        r.includes('signature') ||
                        r.includes('companyid') ||
                        r.includes('forbidden_company');
                return g.NextResponse.json(
                    { ok: !1, error: a ? 'Não autorizado' : e },
                    { status: a ? 401 : 500, headers: t }
                );
            }
        }
        e.s(
        ...
        let T = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/mobile/reviews/dismiss/route',
                    pathname: '/api/mobile/reviews/dismiss',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/mobile/reviews/dismiss/route.ts',
        ...
        async function j(e, t, a) {
            ...
            let g = '/api/mobile/reviews/dismiss/route';
            g = g.replace(/\/index$/, '') || '/';
            let x = await T.prepare(e, t, {
                srcPage: g,
            ...
            let {
                ...
                j = (0, d.normalizeAppPath)(g),
                H = !!(I.dynamicRoutes[j] || I.routes[W]),
                O = async () => (
            ...
            try {
                let i = async (e) =>
                    ...
                    d = async (n) => {
                        ...
                        return (
                            (s && H) || p.delete(m.NEXT_CACHE_TAGS_HEADER),
                            !c.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__2c61256a._.js.map
