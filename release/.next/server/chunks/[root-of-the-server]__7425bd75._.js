module.exports = [
    ...
    (e) => {
        ...
        async function O(e) {
            ...
            try {
                ...
                if (!a || 'object' != typeof a)
                    return E('Body inválido', 400, t);
                let n = ((r = a.email), C(r).toLowerCase()),
                    i = C(a.companyId),
                    s = b(e, 'x-company-id') || b(e, 'x-companyid') || null,
                    o = i || s || '';
                if (!o) return E('companyId é obrigatório', 400, t);
                if (!n || !n.includes('@')) return E('Email inválido', 400, t);
                let l = await R.prisma.company.findUnique({
                    where: { id: o },
                    select: { id: !0, isActive: !0 },
                });
                if (!l || !l.isActive)
                    return E('Empresa inválida ou inativa', 400, t);
                let d =
                        'Se este email estiver cadastrado, enviaremos instruções para redefinir sua senha.',
                    u = await R.prisma.user.findUnique({
                        where: { email: n },
                        select: { id: !0, isActive: !0 },
                    }),
                    p = u?.id
                        ? await R.prisma.companyMember.findUnique({
                              where: {
                                  companyId_userId: {
                                      companyId: o,
                                      userId: u.id,
                        ...
                        : null;
                if (!u || !u.isActive || !p || !p.isActive)
                    return A({ message: d }, 200, t);
                let c = new Date(),
                    h = y.default.randomBytes(32).toString('hex'),
                    m = y.default.createHash('sha256').update(h).digest('hex'),
                    x = new Date(c.getTime() + 12e5);
                if (
                    (await R.prisma.passwordResetToken.updateMany({
                        where: {
                            companyId: o,
                            userId: u.id,
                            usedAt: null,
                    ...
                    await R.prisma.passwordResetToken.create({
                        data: {
                            companyId: o,
                            userId: u.id,
                            tokenHash: m,
                    ...
                    (function (e) {
                        try {
                            let t = new URL(e.url),
                                r = String(
                                    t.searchParams.get('debug') || ''
                                ).trim();
                            if ('1' === r || 'true' === r) return !0;
                        } catch {}
                        let t = String(
                    ...
                    return A(
                        { message: d, token: h, expiresAt: x.toISOString() },
                        200,
                        t
                    );
                return A({ message: d }, 200, t);
                ...
                return (
                    ...
                    E('Erro inesperado', 500, t)
                );
            }
        }
        e.s(['OPTIONS', () => S, 'POST', () => O], 795069);
        var N = e.i(795069);
        let P = new t.AppRouteRouteModule({
                definition: {
                    kind: r.RouteKind.APP_ROUTE,
                    page: '/api/mobile/auth/forgot-password/route',
                ...
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/mobile/auth/forgot-password/route.ts',
        ...
        async function j(e, t, a) {
            ...
            let w = '/api/mobile/auth/forgot-password/route';
            w = w.replace(/\/index$/, '') || '/';
            let R = await P.prepare(e, t, {
                srcPage: w,
            ...
            let {
                ...
                j = !!(T.dynamicRoutes[_] || T.routes[k]),
                U = async () => (
            ...
            try {
                let s = async (e) =>
                    ...
                    l = async (n) => {
                        ...
                        return (
                            (o && j) || p.delete(f.NEXT_CACHE_TAGS_HEADER),
                            !u.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__7425bd75._.js.map
