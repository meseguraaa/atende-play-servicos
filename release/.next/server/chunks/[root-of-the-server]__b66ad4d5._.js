module.exports = [
    ...
    (e) => {
        ...
        async function h(e, r, a) {
            let n, h;
            if (!(0, l.isObject)(e))
                throw new i.JWSInvalid('Flattened JWS must be an object');
            ...
            if (void 0 === e.payload)
                throw new i.JWSInvalid('JWS Payload missing');
            ...
            let y = { ...m, ...e.header },
                ...
                g = !0;
            if (w.has('b64') && 'boolean' != typeof (g = m.b64))
                throw new i.JWSInvalid(
            ...
            if (v && !v.has(f))
                throw new i.JOSEAlgNotAllowed(
                    '"alg" (Algorithm) Header Parameter value not allowed'
                );
            if (g) {
                if ('string' != typeof e.payload)
                    throw new i.JWSInvalid('JWS Payload must be a string');
            } else if (
            ...
            let x = await (0, p.normalizeKey)(r, f);
            if (!(await o(f, x, n, S)))
                throw new i.JWSSignatureVerificationFailed();
            if (g)
                try {
                ...
                h =
                    'string' == typeof e.payload
                        ...
                        : e.payload;
            let H = { payload: h };
            return (void 0 !== e.protected && (H.protectedHeader = m),
        ...
        async function m(e, t, r) {
            ...
            let { 0: a, 1: n, 2: o, length: d } = e.split('.');
            if (3 !== d) throw new i.JWSInvalid('Invalid Compact JWS');
            let l = await h({ payload: n, protected: a, signature: o }, t, r),
    ...
    (e) => {
        ...
        async function o(e, t, o) {
            let i = await (0, n.getSigKey)(e, t, 'sign');
            return (
                (0, a.checkKeyLength)(e, i),
                new Uint8Array(
                    await crypto.subtle.sign(
                        (0, r.subtleAlgorithm)(e, i.algorithm),
                        i,
                        o
                    )
                )
        ...
        class p {
            ...
            async sign(e, r) {
                ...
                if (!(0, i.isDisjoint)(this.#t, this.#r))
                    throw new s.JWSInvalid(
                        'JWS Protected and JWS Unprotected Header Parameter names must be disjoint'
                    );
                let m = { ...this.#t, ...this.#r },
                    ...
                    w = !0;
                if (y.has('b64') && 'boolean' != typeof (w = this.#t.b64))
                    throw new s.JWSInvalid(
                        'The "b64" (base64url-encode payload) Header Parameter must be a boolean'
                    );
                let { alg: g } = m;
                ...
                ((0, l.checkKeyType)(g, e, 'sign'),
                    ...
                    this.#t
                        ? ((p = (0, t.encode)(JSON.stringify(this.#t))),
                          (h = (0, d.encode)(p)))
                        : ((p = ''), (h = new Uint8Array())));
                let f = (0, d.concat)(h, (0, d.encode)('.'), n),
                    v = await (0, u.normalizeKey)(e, g),
                    b = await o(g, v, f),
    ...
    (e) => {
        ...
        async function o(e, t) {
            ...
            return await new r.SignJWT(o)
                .setProtectedHeader({ alg: 'HS256', typ: 'JWT' })
                .setIssuedAt()
                .setExpirationTime(n)
                .sign(a());
        }
        e.s(['signAppJwt', () => o, 'verifyAppJwt', () => n]);
    },
    703051,
    (e) => {
        'use strict';
        ...
        async function H(e, t) {
            let r,
                a = e.headers.get('authorization') || '',
                n = a.startsWith('Bearer ') ? a.slice(7).trim() : '';
            if (
                (console.log(
                    `[mobile/units:${t}] auth header:`,
                    a ? 'present' : 'missing'
                ),
                console.log(
                    `[mobile/units:${t}] token length:`,
                    n ? String(n.length) : '0'
                ),
                !n)
            )
                throw Error('Token ausente');
            let o = await (0, S.verifyAppJwt)(n),
                i = 'string' == typeof o?.sub ? o.sub.trim() : '';
            if (!i) throw Error('Token inválido');
            return {
                sub: i,
                role: o?.role,
                companyId:
                    ((r =
                        'string' == typeof o?.companyId
                            ? String(o.companyId).trim()
                            : '').length
                        ? r
                        : null) ?? void 0,
                email: 'string' == typeof o?.email ? o.email : void 0,
                name: o?.name ?? null,
            };
        }
        async function A() {
        ...
        async function E(e) {
            let t = Math.random().toString(16).slice(2, 8),
                r = Date.now();
            try {
                let a, n;
                (console.log(`
[mobile/units:${t}] ====== GET /api/mobile/units ======`),
                    console.log(`[mobile/units:${t}] url:`, e.url),
                    console.log(
                        `[mobile/units:${t}] time:`,
                        new Date().toISOString()
                    ),
                    console.log(
                        `[mobile/units:${t}] env DATABASE_URL:`,
                        process.env.DATABASE_URL ? 'present' : 'missing'
                    ),
                    console.log(
                        `[mobile/units:${t}] env APP_JWT_SECRET:`,
                ...
                let o = await H(e, t),
                    i =
                        ((a =
                            ...
                            e.headers.get('X-CompanyId')),
                        (n = 'string' == typeof a ? a.trim() : '').length
                            ? n
                            : null),
                    s = i ?? o.companyId ?? null;
                if (
                    (console.log(`[mobile/units:${t}] auth payload:`, {
                        sub: o.sub,
                        role: o.role ?? null,
                        companyIdFromHeader: i ?? null,
                        companyIdFromToken: o.companyId ?? null,
                        resolvedCompanyId: s,
                        email: o.email ?? null,
                    }),
                    !s)
                    ...
                    throw Error(
                        'companyId ausente (header x-company-id ou token)'
                    );
                console.log(
                    `[mobile/units:${t}] querying prisma.unit.findMany...`
                );
                let d = await b.prisma.unit.findMany({
                    where: { companyId: s, isActive: { not: !1 } },
                    orderBy: { name: 'asc' },
                    select: { id: !0, name: !0 },
                });
                return (
                    console.log(`[mobile/units:${t}] prisma returned:`, {
                        count: d.length,
                        sample: d.slice(0, 3),
                    }),
                    console.log(
                        `[mobile/units:${t}] done in(ms):`,
                        Date.now() - r
                    ),
                    v.NextResponse.json(
                        {
                ...
                let e = String(a?.message ?? 'Erro');
                (console.error(`[mobile/units:${t}] ❌ error name:`, a?.name),
                    console.error(`[mobile/units:${t}] ❌ error message:`, e),
                    console.error(
                        `[mobile/units:${t}] ❌ error stack:`,
                        a?.stack
                    ),
                    console.error(`[mobile/units:${t}] ❌ raw:`, a),
                    console.error(`[mobile/units:${t}] prisma code:`, a?.code),
                    console.error(`[mobile/units:${t}] prisma meta:`, a?.meta));
                let r = e.toLowerCase();
                if (
                    r.includes('token') ||
                    r.includes('jwt') ||
                    ...
                    r.includes('não autorizado') ||
                    r.includes('companyid')
                )
        ...
        e.s(['GET', () => E, 'OPTIONS', () => A], 781662);
        var R = e.i(781662);
        let T = new t.AppRouteRouteModule({
                definition: {
                    kind: r.RouteKind.APP_ROUTE,
                    page: '/api/mobile/units/route',
                    pathname: '/api/mobile/units',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath: '[project]/src/app/api/mobile/units/route.ts',
        ...
        async function C(e, t, a) {
            ...
            let {
                ...
                N = !!(R.dynamicRoutes[k] || R.routes[W]),
                U = async () => (
                ...
                K = {
                    ...
                    renderOpts: {
                        ...
                        onInstrumentationRequestError: (t, r, a) =>
                            T.onRequestError(e, t, a, I),
                    },
                    sharedContext: { buildId: S },
                },
                F = new l.NodeNextRequest(e),
            ...
            try {
                let i = async (e) =>
                    ...
                    d = async (n) => {
                        ...
                        if (!N) return null;
                        if (
                            (null == c || null == (o = c.value)
                                ? void 0
                                : o.kind) !== f.CachedRouteKind.APP_ROUTE
                        )
                        ...
                        return (
                            (s && N) || u.delete(w.NEXT_CACHE_TAGS_HEADER),
                            !c.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b66ad4d5._.js.map
