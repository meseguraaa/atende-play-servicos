module.exports = [
    ...
    (a) => {
        ...
        let b = (0, a.i(976286).registerClientReference)(
            function () {
                throw Error(
                    "Attempted to call the default export of [project]/src/app/admin/client-level/rules/ui-client.tsx <module evaluation> from the server, but it's on the client. It's not possible to invoke a client function from the server, it can only be rendered as a Component or passed to props of a Client Component."
                );
            },
    ...
    (a) => {
        ...
        let b = (0, a.i(976286).registerClientReference)(
            function () {
                throw Error(
                    "Attempted to call the default export of [project]/src/app/admin/client-level/rules/ui-client.tsx from the server, but it's on the client. It's not possible to invoke a client function from the server, it can only be rendered as a Component or passed to props of a Client Component."
                );
            },
    ...
    (a) => {
        ...
        async function f({ searchParams: a }) {
            ...
            let h = await a,
                i = h.unitId,
                j = Array.isArray(i) ? i[0] : i,
                k = h.create,
                l = Array.isArray(k) ? k[0] : k,
                m = await (0, c.headers)(),
                n = new URL(
                    '/api/admin/client-levels/rules',
                    ((f = m.get('x-forwarded-proto') ?? 'http'),
                    (g =
                        m.get('x-forwarded-host') ??
                        m.get('host') ??
                        'localhost:3000'),
                    `${f}://${g}`)
                );
            j && n.searchParams.set('unitId', j);
            let o = m.get('cookie') ?? '',
                p = null,
                q = null;
            try {
                ...
                if (a.status >= 300 && a.status < 400)
                    ...
                    (a.headers.get('content-type') ?? '').includes(
                        'application/json'
                    )
                ) {
                    let b = await a.json();
                ...
                } else q = 'Resposta inválida do servidor (esperado JSON).';
            } catch (a) {
                q =
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f21353a2._.js.map
