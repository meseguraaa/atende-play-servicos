module.exports = [
    ...
    (e) => {
        ...
        function A(e, t = 200) {
            return x.NextResponse.json({ ok: !0, data: e }, { status: t });
        }
        async function I(e, t) {
            try {
                let r = await (0, g.requireAdminForModule)('CHECKOUT'),
                    n = r.companyId;
                if (!n) return w('Empresa não encontrada na sessão.', 401);
                let a = r.id;
                if (!a) return w('Usuário não encontrado na sessão.', 401);
                let i = r.canSeeAllUnits,
                    { clientId: o } = await t.params,
                    s = m(o);
                if (!s) return w('clientId é obrigatório.', 400);
                let d = await e.json().catch(() => null),
                    l =
                        d && 'object' == typeof d && Array.isArray(d.orderIds)
                            ? d.orderIds.map((e) => m(e)).filter(Boolean)
                            : [],
                    c = new Date(),
                    u = await E.prisma.order.findMany({
                        where: {
                            companyId: n,
                            clientId: s,
                            id: l.length > 0 ? { in: l } : void 0,
                            status: {
                                in: ['PENDING', 'PENDING_CHECKIN', 'CANCELED'],
                            },
                        },
                        select: {
                            id: !0,
                ...
                if (0 === u.length)
                    return A({
                        ...
                        skippedCount: 0,
                        canceledAt: c.toISOString(),
                    });
                let p = u,
                    h = 0;
                if (!i) {
                    let e = await E.prisma.adminUnitAccess.findMany({
                            where: { companyId: n, userId: a },
                            select: { unitId: !0 },
                        }),
                        t = new Set(e.map((e) => e.unitId)),
                        r = u.filter((e) => t.has(e.unitId));
                    if (((p = r), (h = u.length - r.length), 0 === p.length))
                        return w('Sem acesso às unidades desta conta.', 403);
                }
                let v = p.filter(
                        (e) => 'CANCELED' === e.status && e.inventoryRevertedAt
                    ),
                    C = p.filter(
                        (e) =>
                            'PENDING' === e.status ||
                            'PENDING_CHECKIN' === e.status ||
                            ('CANCELED' === e.status && !e.inventoryRevertedAt)
                    );
                if (0 === C.length)
                    return A({
                        clientId: s,
                        canceledCount: 0,
                        canceledOrderIds: [],
                        alreadyCanceledCount: v.length,
                        skippedCount: h,
                        canceledAt: c.toISOString(),
                    });
                let f = C.map((e) => e.id),
                    R = await E.prisma.$transaction(async (e) => {
                        let t = C.filter((e) => !e.inventoryRevertedAt).map(
                            (e) => e.id
                        );
                        if (t.length > 0) {
                            let r = await e.orderItem.findMany({
                                    where: {
                                        companyId: n,
                                        orderId: { in: t },
                                        productId: { not: null },
                                    },
                                    select: { productId: !0, quantity: !0 },
                                }),
                                a = new Map();
                            for (let e of r) {
                                let t = m(e.productId);
                            ...
                            for (let [t, r] of a.entries())
                                await e.product.updateMany({
                                    where: { id: t, companyId: n },
                                    data: { stockQuantity: { increment: r } },
                                });
                        }
                        let r = await e.order.updateMany({
                                where: {
                                    companyId: n,
                                    clientId: s,
                                    id: { in: f },
                                ...
                                data: {
                                    status: 'CANCELED',
                                    expiredAt: c,
                                    inventoryRevertedAt: c,
                                },
                            }),
                            a = await e.order.updateMany({
                                where: {
                                    companyId: n,
                                    clientId: s,
                                    id: { in: f },
                                ...
                                data: { inventoryRevertedAt: c, expiredAt: c },
                            }),
                            i = await e.order.findMany({
                                where: {
                                    companyId: n,
                                    clientId: s,
                                    id: { in: f },
                        ...
                        return {
                            canceledCount: (r.count ?? 0) + (a.count ?? 0),
                            canceledOrderIds: i.map((e) => e.id),
                            alreadyCanceledCount: v.length,
                        };
                    });
                return A({
                    ...
                    canceledOrderIds: R.canceledOrderIds,
                    alreadyCanceledCount: R.alreadyCanceledCount,
                    skippedCount: h,
                    canceledAt: c.toISOString(),
                });
            } catch (e) {
        ...
        let O = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/admin/checkout/accounts/[clientId]/cancel/route',
                    pathname: '/api/admin/checkout/accounts/[clientId]/cancel',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/admin/checkout/accounts/[clientId]/cancel/route.ts',
        ...
        async function D(e, t, n) {
            ...
            let x = '/api/admin/checkout/accounts/[clientId]/cancel/route';
            x = x.replace(/\/index$/, '') || '/';
            let E = await O.prepare(e, t, {
                srcPage: x,
            ...
            let {
                ...
                M = (0, d.normalizeAppPath)(x),
                q = !!(N.dynamicRoutes[M] || N.routes[k]),
                _ = async () => (
            ...
            if (q && !I) {
                let e = !!N.routes[k],
                    t = N.dynamicRoutes[M];
                if (t && !1 === t.fallback && !e) {
                    if (w.experimental.adapterPath) return await _();
                    throw new R.NoFallbackError();
                }
            }
            let H = null;
            !q || O.isDev || I || (H = '/index' === (H = k) ? '/' : H);
            let j = !0 === O.isDev || !q,
                U = q && !j;
            T &&
                D &&
            ...
            try {
                let o = async (e) =>
                    ...
                    d = async (a) => {
                        ...
                        return (
                            (s && q) || u.delete(f.NEXT_CACHE_TAGS_HEADER),
                            !c.cacheControl ||
                                t.getHeader('Cache-Control') ||
                                u.get('Cache-Control') ||
                                u.set(
                                    'Cache-Control',
                                    (0, C.getCacheControlHeader)(c.cacheControl)
                                ),
                            await (0, h.sendResponse)(
                                G,
                                B,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__ed4ae1fc._.js.map
