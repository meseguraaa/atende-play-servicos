        ...
        async function h(e, r, a) {
            ...
            if (e.protected)
                try {
                    ...
                    w = JSON.parse(s.decoder.decode(r));
                } catch {
                    throw new i.JWSInvalid('JWS Protected Header is invalid');
                }
            if (!(0, d.isDisjoint)(w, e.header))
                throw new i.JWSInvalid(
            ...
            let y = { ...w, ...e.header },
                ...
                m = !0;
            if (f.has('b64') && 'boolean' != typeof (m = w.b64))
                throw new i.JWSInvalid(
            ...
            let x = !1;
            ('function' == typeof r && ((r = await r(w, e)), (x = !0)),
            ...
            let R = await (0, u.normalizeKey)(r, v);
            if (!(await o(v, R, n, S)))
            ...
            if (m)
                ...
                h =
                    'string' == typeof e.payload
                        ...
                        : e.payload;
            let b = { payload: h };
            return (void 0 !== e.protected && (b.protectedHeader = w),
    ...
    (e) => {
        ...
        async function E(e) {
            ...
            let o =
                'string' == typeof a?.companyId
                    ...
                    : '';
            if (!o) throw Error('companyId ausente no token');
            return { sub: n, role: a.role, companyId: o };
        }
        async function A() {
            return new g.NextResponse(null, { status: 204, headers: b() });
        }
        let I = S.z.object({
            appointmentId: S.z.string().min(1, 'Atendimento é obrigatório'),
            rating: S.z.number().int().min(1).max(5),
            comment: S.z
                .string()
                .max(1e3, 'Comentário muito longo')
                .optional()
                .nullable(),
            tagIds: S.z.array(S.z.string()).max(3).optional().default([]),
            isAnonymousForProfessional: S.z.boolean().optional().default(!1),
        });
        async function C(e) {
            let t = b();
            try {
                ...
                let {
                        ...
                        comment: l,
                        tagIds: c,
                        isAnonymousForProfessional: p,
                    } = i.data,
                    u = await R.prisma.appointment.findFirst({
                        where: {
                            id: s,
                            companyId: n,
                            clientId: a,
                ...
                let h = Array.from(new Set(c ?? [])).slice(0, 3);
                await R.prisma.$transaction(async (e) => {
                    let t = await e.appointmentReview.create({
                        data: {
                            companyId: n,
                            appointmentId: u.id,
                            clientId: a,
                    ...
                    if (h.length > 0) {
                        let r = await e.reviewTag.findMany({
                            where: {
                                companyId: n,
                                id: { in: h },
                                isActive: !0,
                        ...
                        r.length > 0 &&
                            (await e.appointmentReviewTag.createMany({
                                data: r.map((e) => ({
                                    reviewId: t.id,
                                    tagId: e.id,
                                })),
                    ...
                    await e.appointment.updateMany({
                        where: { id: u.id, companyId: n },
                        data: { reviewModalShown: !0 },
                    });
                });
                let w = g.NextResponse.json({ ok: !0 }, { headers: t });
                return (w.headers.set('x-company-id', n), w);
            } catch (a) {
                let e = String(
                        a?.message ?? 'Erro ao salvar sua avaliação.'
                    ).toLowerCase(),
                    r =
                        e.includes('token') ||
                        e.includes('jwt') ||
        ...
        let P = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/mobile/reviews/route',
                    pathname: '/api/mobile/reviews',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/mobile/reviews/route.ts',
        ...
        async function H(e, t, a) {
            ...
            let g = '/api/mobile/reviews/route';
            g = g.replace(/\/index$/, '') || '/';
            let x = await P.prepare(e, t, {
                srcPage: g,
            ...
            let {
                ...
                H = (0, d.normalizeAppPath)(g),
                O = !!(I.dynamicRoutes[H] || I.routes[k]),
                j = async () => (
                    ...
                    d = async (n) => {
                        ...
                        let l = async ({ previousCacheEntry: r }) => {
                                try {
                                    ...
                                    let d = z.renderOpts.pendingWaitUntil;
                                    d &&
                                        a.waitUntil &&
                        ...
                        let p = (0, w.fromNodeOutgoingHttpHeaders)(
                            c.value.headers
                        );
                        return (
                            (s && O) || p.delete(f.NEXT_CACHE_TAGS_HEADER),
                            !c.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__269900c6._.js.map
