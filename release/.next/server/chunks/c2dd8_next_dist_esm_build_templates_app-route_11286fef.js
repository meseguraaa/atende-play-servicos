module.exports = [
    ...
    (e) => {
        ...
        async function M(e) {
            return !!(await T.prisma.professionalUnit.findFirst({
                where: {
        ...
        async function x(e) {
            try {
                let t = await (0, g.requireProfessionalSession)(),
                    i = C(t.companyId),
                    a = C(t.professionalId),
                    r = C(t.userId);
                if (!i || !a || !r)
                    return A('Sessão do profissional inválida.', 401);
                let { searchParams: n } = new URL(e.url),
                    s = C(n.get('unitId')),
                    o = C(n.get('professionalId')),
                    l = C(n.get('date')),
                    d = C(n.get('serviceId')),
                    u = C(n.get('appointmentId'));
                if (!s) return A('unitId é obrigatório.', 400);
                if (!l) return A('date é obrigatório (yyyy-MM-dd).', 400);
                if (o && o !== a)
                    return A('professionalId inválido para a sessão.', 403);
                let p = (function (e) {
                    if (!e) return null;
                    let [t, i, a] = e.split('-').map(Number);
                    return t && i && a ? { y: t, m: i, d: a } : null;
                })(l);
                if (!p) return A('date inválido. Use yyyy-MM-dd.', 400);
                if (
                ...
                if (!(await M({ companyId: i, professionalId: a, unitId: s })))
                    return A('Sem acesso a esta unidade.', 403);
                if (
                    u &&
                    !(await T.prisma.appointment.findFirst({
                        where: { id: u, companyId: i, professionalId: a },
                        select: { id: !0 },
                    }))
                )
                    return A('Agendamento inválido para edição.', 404);
                let c = 30;
                if (d) {
                    ...
                    if (!e)
                        return A('Serviço inválido para esta unidade.', 404);
                    c = Math.max(1, Number(e.durationMinutes || 30));
                }
                let f = new Date(Date.UTC(p.y, p.m - 1, p.d, 12, 0, 0, 0)),
                    m = await T.prisma.professionalDailyAvailability.findFirst({
                        where: {
                            companyId: i,
                            professionalId: a,
                            unitId: s,
                ...
                if (m?.type === E.ProfessionalDailyAvailabilityType.DAY_OFF)
                    return I({
                        date: l,
                        unitId: s,
                        professionalId: a,
                ...
                let h = [];
                if (m && m.type === E.ProfessionalDailyAvailabilityType.CUSTOM)
                    h = m.intervals
                        .map((e) => ({
                    ...
                    let e,
                        ...
                        n =
                            ((e = new Date(
                                Date.UTC(p.y, p.m - 1, p.d, 12, 0, 0, 0)
                            )),
                ...
                let v = h.flatMap((e) =>
                    (function (e, t, i) {
                        ...
                        return s;
                    })(e.startTime, e.endTime, c)
                );
                v = Array.from(new Set(v)).sort((e, t) => N(e) - N(t));
        ...
        let P = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/professional/availability/times/route',
                    pathname: '/api/professional/availability/times',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/professional/availability/times/route.ts',
        ...
        async function F(e, t, a) {
            ...
            let R = '/api/professional/availability/times/route';
            R = R.replace(/\/index$/, '') || '/';
            let T = await P.prepare(e, t, {
                srcPage: R,
            ...
            let {
                ...
                _ = !!(b.dynamicRoutes[U] || b.routes[S]),
                F = async () => (
            ...
            if (_ && !C) {
                let e = !!b.routes[S],
                    t = b.dynamicRoutes[U];
                if (t && !1 === t.fallback && !e) {
                    if (I.experimental.adapterPath) return await F();
                    throw new y.NoFallbackError();
                }
            }
            let k = null;
            !_ || P.isDev || C || (k = '/index' === (k = S) ? '/' : k);
            let H = !0 === P.isDev || !_,
                q = _ && !H;
            O &&
                D &&
