module.exports = [
    ...
    (e) => {
        ...
        async function h(e, r, a) {
            ...
            let y = { ...m, ...e.header },
                ...
                w = !0;
            if (f.has('b64') && 'boolean' != typeof (w = m.b64))
                throw new o.JWSInvalid(
            ...
            let E = await (0, p.normalizeKey)(r, v);
            if (!(await i(v, E, n, S)))
            ...
            if (w)
                ...
                h =
                    'string' == typeof e.payload
                        ...
                        : e.payload;
            let x = { payload: h };
            return (void 0 !== e.protected && (x.protectedHeader = m),
            void 0 !== e.header && (x.unprotectedHeader = e.header),
            b)
                ? { ...x, key: E }
                : x;
        }
    ...
    (e) => {
        ...
        var v = e.i(377404),
            ...
            E = e.i(698043);
        function x(e, t) {
            return g.NextResponse.json({ ok: !0, data: e }, t);
        }
        function I(e, t = 400, r) {
        ...
        async function A() {
            let e = await (0, b.cookies)(),
                t = e.get('painel_session')?.value || '';
            if (!t) throw Error('UNAUTHORIZED');
            let { payload: r } = await (0, S.jwtVerify)(
                t,
                (function () {
                    let e = process.env.PAINEL_JWT_SECRET;
                    if (!e) throw Error('PAINEL_JWT_SECRET_NOT_SET');
                    return new TextEncoder().encode(e);
                })()
            );
            if (!r.sub) throw Error('UNAUTHORIZED');
            if (!r.companyId) throw Error('COMPANY_ID_MISSING');
            let a = await E.prisma.user.findUnique({
                where: { id: r.sub },
                select: { id: !0, role: !0, isOwner: !0, isActive: !0 },
            });
            if (!a || !a.isActive || 'ADMIN' !== a.role)
                throw Error('UNAUTHORIZED');
            let n = await E.prisma.companyMember.findUnique({
                where: {
                    companyId_userId: { companyId: r.companyId, userId: r.sub },
                },
                select: { id: !0, role: !0, isActive: !0 },
            });
            if (!n || !n.isActive) throw Error('UNAUTHORIZED');
            let i = !!a.isOwner || 'OWNER' === n.role;
            return { companyId: r.companyId, userId: r.sub, isOwner: i };
        }
        async function _(e, t) {
            ...
            let { unitId: a } = await t.params;
            if (!a) return I('unit_id_required', 400);
            let n = await E.prisma.unit.findFirst({
                where: { id: a, companyId: r.companyId },
                select: {
                    id: !0,
                    name: !0,
                    phone: !0,
            ...
            return n ? x(n) : I('unit_not_found', 404);
        }
        async function T(e, t) {
            var r;
            let a, n, i, o, s, d, l, c, u, p, h;
            try {
                a = await A();
            } catch (e) {
                return I(
            ...
            if (!a.isOwner) return I('forbidden_owner_only', 403);
            let { unitId: m } = await t.params;
            if (!m) return I('unit_id_required', 400);
            let y = null;
            try {
                y = await e.json();
            } catch {
                return I('invalid_json', 400);
            }
            let f = await E.prisma.unit.findFirst({
                where: { id: m, companyId: a.companyId },
                select: {
                    id: !0,
                    name: !0,
                    phone: !0,
            ...
            if (!f) return I('unit_not_found', 404);
            let w = y?.name !== void 0 ? R(y.name) : f.name;
            if (!w) return I('unit_name_required', 400);
            let v = y?.phone !== void 0 ? R(y.phone) : f.phone,
                g = y?.cep !== void 0 ? R(y.cep) : f.cep,
                b = y?.street !== void 0 ? R(y.street) : f.street,
                S = y?.number !== void 0 ? R(y.number) : f.number,
                _ = y?.complement !== void 0 ? R(y.complement) : f.complement,
                T =
                    y?.neighborhood !== void 0
                        ? R(y.neighborhood)
                        : f.neighborhood,
                C = y?.city !== void 0 ? R(y.city) : f.city,
                N = y?.state !== void 0 ? R(y.state) : f.state,
                P = 'boolean' == typeof y?.isActive ? y.isActive : f.isActive,
                W =
                    ((n = []),
                    (i = R(
                        (r = {
                    ...
                    (c = R(r.cep)),
                    (u = R(r.complement)),
                    (p = [i, o ? `, ${o}` : ''].join('').trim()) && n.push(p),
                    u && n.push(u),
                    s && n.push(s),
                    (h = [d, l].filter(Boolean).join(' - ')) && n.push(h),
                    c && n.push(`CEP ${c}`),
                    n.join(' • ') || null);
            try {
                let e = await E.prisma.unit.update({
                    where: { id: m },
                    data: {
                        name: w,
                        phone: v,
        ...
        e.s(
            ['GET', () => _, 'PUT', () => T, 'dynamic', 0, 'force-dynamic'],
            369027
        );
        var C = e.i(369027);
        let N = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/admin/settings/units/[unitId]/route',
                    pathname: '/api/admin/settings/units/[unitId]',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/admin/settings/units/[unitId]/route.ts',
        ...
        async function H(e, t, a) {
            ...
            let g = '/api/admin/settings/units/[unitId]/route';
            g = g.replace(/\/index$/, '') || '/';
            let b = await N.prepare(e, t, {
                srcPage: g,
            ...
            let {
                ...
                H = !!(A.dynamicRoutes[O] || A.routes[P]),
                U = async () => (
            ...
            try {
                let o = async (e) =>
                    ...
                    d = async (n) => {
                        ...
                        return (
                            (s && H) || u.delete(f.NEXT_CACHE_TAGS_HEADER),
                            !c.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__d7e83a42._.js.map
