module.exports = [
    ...
    (e) => {
        ...
        var t = e.i(698043),
            r = e.i(52359);
        async function n() {
            let e = await (0, r.getCurrentPainelUser)();
        ...
        e.s(['requireProfessionalSession', () => n]);
    },
    725525,
    (e) => {
        'use strict';
        ...
        async function E(t) {
            try {
                let r,
                    n = (function () {
                        try {
                            let t = e.r(896407),
                                r =
                                    t?.requireProfessionalSession ??
                                    t?.default ??
                                    t?.requireProfessional;
                ...
                let a = await n(),
                    i = a?.companyId,
                    o = a?.professionalId;
                if (!i) return A('Sessão sem companyId.', 401);
                if (!o) return A('Sessão sem professionalId.', 401);
                let s = t.nextUrl.searchParams.get('month'),
                    l =
                        (function (e) {
                            if (!e) return null;
                            let t = String(e).trim();
                    ...
                    u = new Date(l.getFullYear(), l.getMonth(), 1, 0, 0, 0, 0),
                    d = new Date(
                        l.getFullYear(),
                        l.getMonth() + 1,
                        0,
                    ...
                    c =
                        (r = new Intl.DateTimeFormat('pt-BR', {
                            month: 'long',
                            year: 'numeric',
                        }).format(l))
                            .charAt(0)
                    ...
                    [p, m] = await Promise.all([
                        y.prisma.appointmentReview.findMany({
                            where: {
                                companyId: i,
                                professionalId: o,
                                createdAt: { gte: u, lte: d },
                            },
                            include: {
                                client: !0,
                                appointment: { include: { service: !0 } },
                                tags: { include: { tag: !0 } },
                            },
                            orderBy: { createdAt: 'desc' },
                        }),
                        y.prisma.appointmentReview.findMany({
                            where: { companyId: i, professionalId: o },
                            select: { rating: !0 },
                        }),
                    ]),
                    f = p.map((e) => {
                        let t = e.isAnonymousForProfessional
                                ? 'Cliente (anônimo)'
                                : (e.client?.name ?? 'Cliente'),
                            r = e.appointment?.service?.name ?? 'Atendimento',
                            n = (e.tags ?? [])
                                .map((e) => e.tag?.label)
                                .filter(Boolean);
                        return {
                    ...
                    h = p.length,
                    g = h > 0 ? p.reduce((e, t) => e + t.rating, 0) / h : 0,
                    w = m.length,
                    R = w > 0 ? m.reduce((e, t) => e + t.rating, 0) / w : 0,
                    E = new Map(),
                    I = new Map();
                for (let e of p) {
                    let t = e.rating >= 3,
                    ...
                    for (let n of e.tags ?? []) {
                        let e = n.tag?.label;
                        e &&
                            (t && E.set(e, (E.get(e) ?? 0) + 1),
                            r && I.set(e, (I.get(e) ?? 0) + 1));
                    }
                }
                let C = Array.from(E.entries())
                    ...
                    S = Array.from(I.entries())
                        ...
                        .slice(0, 8),
                    x = f.filter((e) => e.rating >= 3),
                    b = f.filter((e) => e.rating <= 2),
                    P = [...x]
                        .sort(
                            (e, t) =>
                ...
                return (function (e, t = 200) {
                    ...
                    month: {
                        value: `${l.getFullYear()}-${String(l.getMonth() + 1).padStart(2, '0')}`,
                        label: c,
                        start: u.toISOString(),
                        end: d.toISOString(),
                    },
                    stats: {
                        totalReviewsMonth: h,
                        averageRatingMonth: g,
                        totalReviewsOverall: w,
                        averageRatingOverall: R,
                    },
                    topPositiveTags: C,
                    topNegativeTags: S,
                    recentPositiveReviews: P,
                    recentNegativeReviews: T,
                    reviews: f,
                });
            } catch (e) {
                return (
                    console.error('GET /api/professional/review error:', e),
                    A('Erro inesperado ao carregar avaliações.', 500)
                );
            }
        }
        e.s(['GET', () => E], 585270);
        ...
        let C = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/professional/review/route',
                    pathname: '/api/professional/review',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/professional/review/route.ts',
        ...
        async function T(e, t, n) {
            ...
            let v = '/api/professional/review/route';
            v = v.replace(/\/index$/, '') || '/';
            let y = await C.prepare(e, t, {
                srcPage: v,
            ...
            let {
                ...
                F = (0, l.normalizeAppPath)(v),
                U = !!(b.dynamicRoutes[F] || b.routes[O]),
                _ = async () => (
            ...
            try {
                let o = async (e) =>
                    ...
                    l = async (a) => {
                        ...
                        return (
                            (s && U) || c.delete(g.NEXT_CACHE_TAGS_HEADER),
                            !d.cacheControl ||
