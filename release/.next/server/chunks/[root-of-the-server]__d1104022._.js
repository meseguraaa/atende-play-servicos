    ...
    (e) => {
        ...
        async function h(e, r, a) {
            ...
            let m = { ...y, ...e.header },
                ...
                w = !0;
            if (f.has('b64') && 'boolean' != typeof (w = y.b64))
                throw new o.JWSInvalid(
        ...
        class u {
            ...
            async sign(e, r) {
                ...
                let y = { ...this.#t, ...this.#r },
                    ...
                    f = !0;
                if (m.has('b64') && 'boolean' != typeof (f = this.#t.b64))
                    throw new s.JWSInvalid(
                        'The "b64" (base64url-encode payload) Header Parameter must be a boolean'
                    );
                let { alg: w } = y;
    ...
    (e) => {
        ...
        async function E(e) {
            try {
                ...
                if (!i || 'object' != typeof i) return H('Body inválido');
                let n = R(i.name),
                    o = R(i.email).toLowerCase(),
                    s = R(i.phone),
                    d = R(i.birthday),
                    l = R(i.password),
                    c = R(i.companyId);
                if (!c) return H('companyId é obrigatório');
                if (!n || n.length < 2) return H('Nome inválido');
                if (!o || !o.includes('@')) return H('Email inválido');
                if (!A.test(String(l || '')))
                    return H(
                        'A senha deve ter no mínimo 6 caracteres, incluindo 1 letra maiúscula, 1 número e 1 caractere especial.'
                    );
                let p = s.replace(/\D+/g, '');
                if (11 !== p.length) return H('Telefone inválido');
                let u = (function (e) {
                    let [t, r, a] = String(e || '').split('/');
                ...
                if (!u) return H('Data de nascimento inválida');
                let h = await b.prisma.company.findUnique({
                    where: { id: c },
                    select: { id: !0, isActive: !0 },
                });
                if (!h || !h.isActive) return H('Empresa inválida ou inativa');
                if (
                    await b.prisma.user.findUnique({
                        where: { email: o },
                        select: { id: !0 },
                    })
                )
                    return H('Já existe uma conta com este email');
                let y = await x.default.hash(l, 10),
                    m = await b.prisma.user.create({
                        data: {
                            name: n,
                            email: o,
                            phone: p,
                ...
                await b.prisma.companyMember.create({
                    data: { companyId: c, userId: m.id, role: 'CLIENT' },
                });
                let f =
                        ((t = {
                            phone: m.phone ?? null,
                        ...
                        (r =
                            'string' == typeof t.phone &&
                            t.phone.trim().length > 0),
                        (a =
                            t.birthday instanceof Date &&
                            !Number.isNaN(t.birthday.getTime())),
                        r && a),
                    w = await (0, S.signAppJwt)({
                        sub: m.id,
                        role: 'CLIENT',
                        companyId: c,
                        email: m.email,
                ...
                return (
                    ...
                    H('Erro inesperado ao criar conta', 500)
                );
            }
        }
        e.s(['POST', () => E], 954808);
        ...
        let J = new t.AppRouteRouteModule({
                definition: {
                    ...
                    page: '/api/mobile/auth/signup/route',
                    pathname: '/api/mobile/auth/signup',
                    filename: 'route',
                    bundlePath: '',
                },
                distDir: '.next',
                relativeProjectDir: '',
                resolvedPagePath:
                    '[project]/src/app/api/mobile/auth/signup/route.ts',
        ...
        async function N(e, t, a) {
            ...
            let v = '/api/mobile/auth/signup/route';
            v = v.replace(/\/index$/, '') || '/';
            let b = await J.prepare(e, t, {
                srcPage: v,
            ...
            let {
                ...
                U = !!(E.dynamicRoutes[j] || E.routes[P]),
                k = async () => (
                    (null == T ? void 0 : T.render404)
                        ? await T.render404(e, t, R, !1)
            ...
            try {
                let o = async (e) =>
                    ...
                    d = async (i) => {
                        ...
                        return (
                            (s && U) || p.delete(f.NEXT_CACHE_TAGS_HEADER),
                            !c.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__d1104022._.js.map
