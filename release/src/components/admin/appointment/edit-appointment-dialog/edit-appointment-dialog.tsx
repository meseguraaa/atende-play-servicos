// src/components/admin/appointment/edit-appointment-dialog/edit-appointment-dialog.tsx
'use client';
...
function getTimeHHmm(value: string | Date): string {
...
export default function EditAppointmentDialog({
    ...
    const currentUnitLabel = useMemo(() => {
        ...
        return activeUnits.find((u) => u.id === unitId)?.name ?? null;
    }, [unitId, activeUnits]);
    ...
    React.useEffect(() => {
        ...
        // hidrata apenas 1x por abertura/appointment id
        if (hydratedRef.current === appt.id) return;
        hydratedRef.current = appt.id;

        const apptDate = getDateOnly(appt.scheduleAt);
        const apptTime = getTimeHHmm(appt.scheduleAt);

        setSelectedClientId(appt.clientId ?? '');
        setClientName(appt.clientName ?? '');
        setPhone(appt.phone ?? '');
        setClientQuery(
            `${appt.clientName ?? ''}${appt.phone ? ` • ${appt.phone}` : ''}`
        );
    ...
    React.useEffect(() => {
        if (!open) return;
        // se fechar, libera pra rehidratar no próximo open
        return () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (!res.ok) {
                toast.error(
                    data?.error ?? 'Não foi possível editar o agendamento.'
                );
                return;
            }
    ...
    const shouldLockUnit =
        !!forcedUnitId || activeUnits.length <= 1 || activeUnits.length === 0;

    const lockedUnitLabel = forcedUnitId
        ? forcedUnitLabel
        : (currentUnitLabel ?? 'Unidade');
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            >
                <DialogHeader>
                    <DialogTitle size="modal">Editar agendamento</DialogTitle>
