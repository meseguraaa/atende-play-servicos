...
function appointmentServiceRevenue(row: {
    ...
    service: { name: string; price: any; professionalPercentage: any } | null;
...
// Comissão de serviço:
// - Preferência absoluta: professionalEarningValue (já “congelado”)
// - Senão: servicePriceAtTheTime * professionalPercentageAtTheTime
// - Senão: service.price * service.professionalPercentage
function appointmentServiceCommission(row: {
    professionalEarningValue: any | null;
    professionalPercentageAtTheTime: any | null;
    servicePriceAtTheTime: any | null;
    service: { price: any; professionalPercentage: any } | null;
    order: { status: OrderStatus; totalAmount: any } | null;
}): number {
    const earning = toNumberDecimal(row.professionalEarningValue);
    if (Number.isFinite(earning) && earning > 0) return earning;

    const priceAtTheTime = toNumberDecimal(row.servicePriceAtTheTime);
    const pctAtTheTime = toNumberDecimal(row.professionalPercentageAtTheTime);
    ...
    const basePct = toNumberDecimal(row.service?.professionalPercentage);
...
// Comissão de produto = totalPrice * (product.professionalPercentage / 100)
function productCommission(
    totalPrice: any,
    professionalPercentage: any
): number {
    const total = toNumberDecimal(totalPrice);
    const pctP = toNumberDecimal(professionalPercentage);
...
export default async function AdminReportsRevenuePage({
    ...
    const baseAppointmentsWhere = {
        ...
        ...(professionalIdSafe ? { professioanlId: professionalIdSafe } : {}),
    };
    ...
    const [
        ...
        prisma.productSale.findMany({
            ...
            select: {
                ...
                totalPrice: true,
                product: {
                    select: { name: true, professionalPercentage: true },
                },
            },
            orderBy: { soldAt: 'asc' },
        }),
    ...
    const baseProductCommission = baseProductSales
        .map((s) =>
            productCommission(s.totalPrice, s.product?.professionalPercentage)
        )
        .reduce((acc, v) => acc + v, 0);
    ...
    for (const a of baseAppointments) {
        const bId = a.professionalId ?? 'unknown';
        const prev = professionalAgg.get(bId) ?? {
            serviceRevenue: 0,
            doneCount: 0,
            serviceCommission: 0,
            productRevenue: 0,
        ...
        professionalAgg.set(bId, prev);
    }

    for (const s of baseProductSales) {
        const bId = s.professionalId ?? 'unknown';
        const prev = professionalAgg.get(bId) ?? {
            serviceRevenue: 0,
            doneCount: 0,
            serviceCommission: 0,
        ...
        prev.productCommission += productCommission(
            s.totalPrice,
            s.product?.professionalPercentage
        );

        professionalAgg.set(bId, prev);
    }

    let professionalRows: ProfessionalRow[] = Array.from(
        professionalAgg.entries()
    ).map(([professionalIdKey, v]) => {
        ...
        return {
            professionalId: professionalIdKey,
            professionalName:
                professionalIdKey === 'unknown'
                    ? '—'
                    : (professionalNameById.get(professionalIdKey) ?? '—'),
    ...
    professionalRows.sort((a, b) => b.totalRevenue - a.totalRevenue);
    ...
    if (baseDoneCount === 0 && baseStandaloneRevenue === 0) {
        ...
        if (!professionalIdSafe && professionalRows.length > 0) {
            const top = professionalRows[0];
            insights.push(
                `Top do mês: ${top.professionalName} (${formatMoneyBRL(top.totalRevenue)}).`
            );
        }
    }
